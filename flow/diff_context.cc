...
Damage DiffContext::GetDamage(
    const SkIRect& accumulated_framebuffer_damage) const {
  SkRect framebuffer = SkRect::Make(accumulated_framebuffer_damage);
  ...
  Damage res;
  framebuffer.roundOut(&res.framebuffer_damage);
  net.roundOut(&res.net_damage);
  return res;
}
