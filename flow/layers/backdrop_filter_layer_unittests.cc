...
TEST_F(BackdropFilterLayerTest, OpacityInheritance) {
  ...
  const DlPath mock_path = DlPath().AddRect(DlFRect::MakeLTRB(0, 0, 10, 10));
  const DlPaint mock_paint = DlPaint(DlColor::kRed());
  const DlFRect clip_rect = DlFRect::MakeLTRB(0, 0, 100, 100);

  auto clip = std::make_shared<ClipRectLayer>(clip_rect, Clip::hardEdge);
  auto parent = std::make_shared<OpacityLayer>(128, DlFPoint(0, 0));
...
TEST_F(BackdropLayerDiffTest, BackdropLayer) {
  ...
  auto clip = std::make_shared<ClipRectLayer>(DlFRect::MakeLTRB(20, 20, 60, 60),
                                              Clip::hardEdge);
  ...
  auto path1 = DlPath().AddRect(DlFRect::MakeLTRB(180, 180, 190, 190));
  ...
  auto path2 = DlPath().AddRect(DlFRect::MakeLTRB(179, 179, 189, 189));
