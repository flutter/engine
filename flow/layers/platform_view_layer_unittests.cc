...
TEST_F(PlatformViewLayerTest, NullViewEmbedderDoesntPrerollCompositeOrPaint) {
  const DlFPoint layer_offset = DlFPoint();
  const DlFSize layer_size = DlFSize(8.0f, 8.0f);
  ...
  EXPECT_EQ(layer->paint_bounds(),
            DlFRect::MakeSize(layer_size).MakeOffset(layer_offset));
...
TEST_F(PlatformViewLayerTest, ClippedPlatformViewPrerollsAndPaintsNothing) {
  const DlFPoint layer_offset = DlFPoint();
  const DlFSize layer_size = DlFSize(8.0f, 8.0f);
  const DlFRect child_clip = DlFRect::MakeLTRB(20.0f, 20.0f, 40.0f, 40.0f);
  const DlFRect parent_clip = DlFRect::MakeLTRB(50.0f, 50.0f, 80.0f, 80.0f);
  const int64_t view_id = 0;
  auto layer =
      std::make_shared<PlatformViewLayer>(layer_offset, layer_size, view_id);
...
TEST_F(PlatformViewLayerTest, StateTransfer) {
  const DlTransform transform1 = DlTransform::MakeTranslate(5, 5);
  const DlTransform transform2 = DlTransform::MakeTranslate(15, 15);
  const DlTransform combined_transform = DlTransform::MakeTranslate(20, 20);
  ...
  auto embedder = MockViewEmbedder();
  DisplayListBuilder builder(DlFRect::MakeWH(500, 500));
