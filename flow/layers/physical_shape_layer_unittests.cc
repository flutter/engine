...
class PhysicalShapeLayerTestMockCanvas : public testing::MockCanvas {
 public:
  ...
  PhysicalShapeLayerTestMockCanvas(int width, int height)
 ...
 private:
  ...
  void onDrawShadowRec(const SkPath& path,
                       const SkDrawShadowRec& rec) override {
...
TEST_F(PhysicalShapeLayerTest, ElevationSimple) {
  ...
  EXPECT_TRUE(layer->needs_painting());
  EXPECT_EQ(layer->needs_system_composite(),
            PhysicalShapeLayerBase::should_system_composite());
