...
TEST_F(ShellTest, CacheSkSLsWorks) {
  ...
  fml::VisitFilesRecursively(base_dir.fd(), skp_visitor);
  EXPECT_GT(skp_count, 0);
  ...
  skp_count = 0;
  fml::VisitFilesRecursively(base_dir.fd(), skp_visitor);
  EXPECT_EQ(skp_count, old_skp_count);
  ...
  fml::VisitFiles(base_dir.fd(), remove_visitor);
  DestroyShell(std::move(shell));
}
...
TEST_F(ShellTest, OnServiceProtocolGetSkSLsWorks) {
  ...
  auto sksl_dir = fml::CreateDirectory(
      base_dir.fd(), {version.str(), PersistentCache::kSkSLSubdirName},
