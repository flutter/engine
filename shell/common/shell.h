...
/// is embedder responsibility to create threads and give the shell task runners
/// for those threads. Due to deterministic destruction of the shell, the
/// embedder can terminate all threads immediately after collecting the shell.
/// The shell must be created and destroyed on the same thread but different
/// shells (i.e. a separate instance of a Flutter application) may be run on
/// different threads simultaneously. The task runners themselves do not have to
/// be unique. If all task runner references given to the shell during shell
/// creation point to the same task runner, the Flutter application is
/// effectively single threaded.
///
/// The shell is the central nervous system of the Flutter application. None of
/// the shell components are thread safe and may only be created, accessed and
...
///
/// All shell methods access by the embedder may only be called on the platform
/// task runner. In case the embedder wants to directly access a shell
/// subcomponent, it is embedder responsibility to acquire a weak pointer to
/// that component and post a task to the task runner used by the component to
/// access its methods.
///
...
class Shell final : public PlatformView::Delegate,
 ...
 public:
  ...
  //----------------------------------------------------------------------------
  /// @brief      Create a shell instance using the provided settings. The
  ...
  ///                                      precendence over any snapshots
  ...
  ///             before performing any operaation on that component. This
  ///             accessor allows callers to acces the task runners for this
  ///             shell.
  ///
  ...
  //----------------------------------------------------------------------------
  /// @brief      Used by embedder to check if all shell subcomponents are
  ///             initialized. It is embedder responsibility to make this call
  ///             before accessing any other shell method. A shell that is not
  ///             setup must be disarded and another one created with updated
  ///             settings.
  ///
  /// @return     Returns if the shell has been setup. Once setup, this does not
  ///             change for the lifecycle of the shell.
  ///
  bool IsSetup() const;

  //----------------------------------------------------------------------------
  /// @brief      Capture a screenshot and optionally Base64 encode the data of
  ///             the last layer tree rendered by the rasterizer in this shell.
  ///
