...
@interface FlutterEngine () <FlutterEmbedderAPIBridgeDelegate>
...
// /**
//  * Loads the AOT snapshots and instructions from the elf bundle (app_elf_snapshot.so) into
//  _aotData,
...
@implementation FlutterEngine {
  ...
  // FLUTTER_API_SYMBOL(FlutterEngine) _embedderAPIBridge.engine;
  ...
  FlutterViewEngineProvider* _engineViewProvider;
}
...
- (instancetype)initWithName:(NSString*)labelPrefix
  ...
  self = [super init];
  NSAssert(self, @"Super init cannot be nil");
  _engineViewProvider = [[FlutterViewEngineProvider alloc] initWithEngine:self];
  _embedderAPIBridge = [[FlutterEmbedderAPIBridge alloc] initWithProject:project
                                                               presenter:_engineViewProvider];
  _embedderAPIBridge.delegate = self;

  // _embedderAPIBridge.project = project ?: [[FlutterDartProject alloc] init];
  ...
  // _renderer = [[FlutterRenderer alloc] initWithEmbedderAPIBridge:self];
...
- (BOOL)runWithEntrypoint:(NSString*)entrypoint {
  ...
  FlutterEngineResult result = [_embedderAPIBridge initializeEmbedderAPIAndRun:entrypoint];
  if (result != kSuccess) {
...
//     return
//     reinterpret_cast<flutter::FlutterCompositor*>(user_data)->Present(kFlutterDefaultViewId,
//                                                                              layers,
//                                                                              layers_count);
//   };
...
- (id<FlutterBinaryMessenger>)binaryMessenger {
  ...
  return _embedderAPIBridge;
}

#pragma mark - Framework-internal methods

- (BOOL)running {
  return [_embedderAPIBridge running];
}
...
- (void)updateDisplayConfig {
  ...
  if (!(nominal.flags & kCVTimeIsIndefinite)) {
    ...
    [_embedderAPIBridge notifyDisplayUpdate:mainDisplayID
                                refreshRate:round(refreshRate)
                                 updateType:kFlutterEngineDisplaysUpdateTypeStartup];
  }
...
- (void)updateWindowMetrics {
  if (!_embedderAPIBridge.engine || !self.viewController.viewLoaded) {
    return;
  }
  ...
  [_embedderAPIBridge SendWindowMetricsEventWithWidth:static_cast<size_t>(scaledSize.width)
...
- (void)sendPointerEvent:(const FlutterPointerEvent&)event {
  [_embedderAPIBridge sendPointerEvent:event];
}

- (void)sendKeyEvent:(const FlutterKeyEvent&)event
            callback:(FlutterKeyEventCallback)callback
            userData:(void*)userData {
  [_embedderAPIBridge sendKeyEvent:event callback:callback userData:userData];
}
...
- (void)setSemanticsEnabled:(BOOL)enabled {
  ...
  [_embedderAPIBridge updateSemanticsEnabled:_semanticsEnabled];
}

- (void)dispatchSemanticsAction:(FlutterSemanticsAction)action
                       toTarget:(uint16_t)target
                       withData:(fml::MallocMapping)data {
  [_embedderAPIBridge dispatchSemanticsAction:action
                                     toTarget:target
                                     withData:data.GetMapping()
                                         size:data.GetSize()];
}
...
- (void)sendUserLocales {
  [_embedderAPIBridge sendUserLocales];
}
...
//       }
//       if (responseHandle) {
//         sendPlatformMessageResponse(weakSelf->_embedderAPIBridge.engine, responseHandle,
 ...
 */
- (void)shutDownEngine {
  // if (_embedderAPIBridge.engine == nullptr) {
  //   return;
  // }

  [self.viewController.flutterView shutdown];

  // FlutterEngineResult result = _embedderAPI.Deinitialize(_embedderAPIBridge.engine);
  // if (result != kSuccess) {
  ...
  // result = _embedderAPI.Shutdown(_embedderAPIBridge.engine);
  // if (result != kSuccess) {
  //   NSLog(@"Failed to shut down Flutter engine: error %d", result);
  // }
  // _embedderAPIBridge.engine = nullptr;
  [_embedderAPIBridge shutDownBridge];
}
...
//         _embedderAPIBridge.engine, message_reply, captures.get(), &response_handle);
...
//   _embedderAPI.SendPlatformMessage(_embedderAPIBridge.engine, &platformMessage); if
//   (message_result != kSuccess) {
...
//         _embedderAPI.PlatformMessageReleaseResponseHandle(_embedderAPIBridge.engine,
//         response_handle);
...
//   if (_embedderAPIBridge.engine) {
//     auto result = _embedderAPI.RunTask(_embedderAPIBridge.engine, &task);
//     if (result != kSuccess) {
...
- (FlutterRenderer*)renderer {
  return _embedderAPIBridge.renderer;
}

#pragma mark - FlutterEmbedderAPIBridgeDelegate
...
- (bool)compositorCreatingBackingStore:(const FlutterBackingStoreConfig*)config
  ...
  return _macOSCompositor->CreateBackingStore(config, backingStoreOut);
}
