...
- (BOOL)runWithEntrypoint:(NSString*)entrypoint {
  ...
  flutter::FlutterMacOSCompositor* mc = new flutter::FlutterMacOSCompositor(SkISize::Make(500, 500), context, _viewController);
  ...
  mc->SetPresentCallback([present = rendererConfig.open_gl.present,
                          user_data = (__bridge void*)(self)]
                          (const FlutterLayer** layers, 
                            size_t layers_count) {
                              if (!present(user_data)) {
                                NSLog(@"err");
                              }
                            }
  );
