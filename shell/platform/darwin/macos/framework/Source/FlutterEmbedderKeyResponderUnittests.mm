...
TEST(FlutterEmbedderKeyResponderUnittests, NonAsciiKeysShouldUseUsEnLogicalKeys) {
  ...
  [responder handleEvent:keyEvent(NSEventTypeKeyDown, 0x100, @"ф", @"ф", FALSE, kKeyCodeKeyA)
                callback:^(BOOL handled){
                }];
  ...
  EXPECT_EQ(event->physical, kPhysicalKeyA);
  EXPECT_EQ(event->logical, kLogicalKeyA);
  EXPECT_STREQ(event->character, "ф");
  EXPECT_EQ(event->synthesized, false);

  [events removeAllObjects];

  [responder handleEvent:keyEvent(NSEventTypeKeyUp, 0x100, @"ф", @"ф", FALSE, kKeyCodeKeyA)
                callback:^(BOOL handled){
                }];
