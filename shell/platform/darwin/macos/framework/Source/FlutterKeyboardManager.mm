...
- (void)handleEvent:(nonnull NSEvent*)event {
  ...
  NSLog(@"Type %lu moF %lu winN %ld c %@ cIM %@ rep %d keyC %x", (unsigned long)event.type,
        (unsigned long)event.modifierFlags, (long)event.windowNumber, event.characters,
        event.charactersIgnoringModifiers, event.isARepeat, event.keyCode);
