...
bool SystemVersionEqualTo(NSOperatingSystemVersion target) {
  ...
  return target.majorVersion == systemVersion.majorVersion &&
         target.minorVersion == systemVersion.minorVersion;
}
...
void FlutterAccessibilityMac::OnAccessibilityEvent(AXEventGenerator::TargetedEvent targeted_event) {
  ...
  switch (event_type) {
    ...
    case ax::AXEventGenerator::Event::SELECTED_CHILDREN_CHANGED:
      if (ax::IsTableLike(GetData().role)) {
        ...
        if (native_accessibility_node.accessibilityFocusedUIElement &&
            ...
            !IsInGeneratedEventBatch(ax::AXEventGenerator::Event::ACTIVE_DESCENDANT_CHANGED) &&
            !IsInGeneratedEventBatch(ax::AXEventGenerator::Event::FOCUS_CHANGED)) {
    ...
    case ax::AXEventGenerator::Event::DOCUMENT_SELECTION_CHANGED: {
      ...
      if (!focus_node)
        break;  // Just fire a notification on the root.
      FireNativeMacNotification(focus_node->GetNativeViewAccessible(), mac_notification);
      break;
    }
    ...
    case ax::AXEventGenerator::Event::VALUE_IN_TEXT_FIELD_CHANGED:
      ...
      if (SystemVersionGreaterOrEqualTo({.majorVersion = 10, .minorVersion = 11}) &&
        ...
        if (user_info) {
          FireNativeMacNotificationWithUserInfo(native_node, mac_notification, user_info);
          FireNativeMacNotificationWithUserInfo(
              GetBridge()->GetFlutterAccessibilityFromID(kRootNode)->GetNativeViewAccessible(),
              mac_notification, user_info);
        }
        return;
      }
      break;
    ...
    case ax::AXEventGenerator::Event::ALERT: {
      FireNativeMacNotification(native_node, NSAccessibilityLiveRegionCreatedNotification);
    ...
    case ax::AXEventGenerator::Event::LIVE_REGION_CHANGED: {
      if (SystemVersionLessOrEqualTo({.majorVersion = 10, .minorVersion = 13})) {
        ...
        FireNativeMacNotificationWithUserInfo([NSApp mainWindow],
      ...
      std::async(std::launch::async, [&native_node, this]() {
        ...
        FireNativeMacNotification(native_node, NSAccessibilityLiveRegionChangedNotification);
      });
      return;
    }
    ...
    case ax::AXEventGenerator::Event::EXPANDED:
      if (GetData().role == ax::Role::kRow || GetData().role == ax::Role::kTreeItem) {
...
void FlutterAccessibilityMac::FireNativeMacNotification(gfx::NativeViewAccessible native_node,
...
void FlutterAccessibilityMac::DispatchAccessibilityAction(uint16_t target,
                                                          FlutterSemanticsAction action,
                                                          uint8_t* data,
                                                          size_t data_size) {
  [GetFlutterEngine() dispatchSemanticsAction:target action:action data:data dataSize:data_size];
}
...
SkRect FlutterAccessibilityMac::GetBoundsRect(const AXCoordinateSystem coordinate_system,
  ...
  SkRect bounds = GetBridge()->GetAXTree()->RelativeToTreeBounds(
  ...
  SkRect result = SkRect::MakeXYWH(global_bounds.origin.x, global_bounds.origin.y,
                                   global_bounds.size.width, global_bounds.size.height);
  return result;
}
...
std::string FlutterAccessibilityMac::GetLiveRegionText() const {
  ...
  for (int32_t child : GetData().child_ids) {
    const FlutterAccessibilityMac* accessibility_child =
        (FlutterAccessibilityMac*)GetBridge()->GetFlutterAccessibilityFromID(child);
...
void FlutterAccessibilityMac::computeTextEdit(std::u16string& inserted_text,
                                              std::u16string& deleted_text,
                                              id* edit_text_marker) {
  ...
  // whitespace.
  for (i = 0; i < old_value.length() && i < new_value.length() &&
              (old_value[i] == new_value[i] || (iswspace(old_value[i]) && iswspace(new_value[i])));
       ++i) {
  }
  for (j = 0; (i + j) < old_value.length() && (i + j) < new_value.length() &&
              (old_value[old_value.length() - j - 1] == new_value[new_value.length() - j - 1] ||
  ...
  AXTextMarkerRef cf_text_marker =
      AXTextMarkerCreate(kCFAllocatorDefault, reinterpret_cast<const UInt8*>(&serialized),
                         sizeof(AXNodePosition::SerializedPosition));
  *edit_text_marker = (__bridge id)(cf_text_marker);
  return;
}

NSDictionary* FlutterAccessibilityMac::GetUserInfoForValueChangedNotification(
  ...
  if (!deleted_text.empty()) {
    ...
    NSMutableDictionary* change = [NSMutableDictionary dictionaryWithDictionary:@{
      NSAccessibilityTextEditType : @(AXTextEditTypeDelete),
      NSAccessibilityTextChangeValueLength : @(deleted_text.length()),
      NSAccessibilityTextChangeValue : [NSString stringWithCharacters:p_u16chars
                                                               length:deleted_text.size()]
    }];
    if (edit_text_marker) {
  ...
  if (!inserted_text.empty()) {
    ...
    auto edit_type = inserted_text.length() > 1 ? @(AXTextEditTypeInsert) : @(AXTextEditTypeTyping);
