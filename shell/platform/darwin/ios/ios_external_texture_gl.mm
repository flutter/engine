...
void IOSExternalTextureGL::Paint(SkCanvas& canvas, const SkRect& bounds, bool freeze, GrContext* context) {
  if (!cache_ref_) {
  ...
  sk_sp<SkImage> image = SkImage::MakeFromTexture(context == nullptr ? canvas.getGrContext() : context, backendTexture, kTopLeft_GrSurfaceOrigin,
