...
static std::unique_ptr<shell::Rasterizer> CreateHeadlessRasterizer(shell::Shell& shell) {
  return std::make_unique<shell::NullRasterizer>(shell.GetTaskRunners());
}
...
- (void)runWithEntrypoint:(NSString*)entrypoint {
  ...
  shell::RunConfiguration run_configuration =
      shell::RunConfiguration::DefaultRunConfigurationFromSettings(settings);

  // Override the default run configuration with the specified entrypoint.
  _shell->GetTaskRunners().GetUITaskRunner()->PostTask(
      [ engine = _shell->GetEngine(), config = std::move(run_configuration) ]() {
        if (!engine || !engine->Run(config)) {
          FXL_LOG(ERROR) << "Could not launch engine with configuration: " << config.ToString();
        }
      });
}
