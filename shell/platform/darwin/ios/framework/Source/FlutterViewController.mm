...
static void FirstFrameCallback(void* userData) {
  FlutterViewController* viewController = (__bridge FlutterViewController*)userData;
  fml::RefPtr<fml::TaskRunner> platformTaskRunner = viewController.engine.platformTaskRunner;
  // fml::RefPtr<fml::TaskRunner> RasterTaskRunner = viewController.engine.RasterTaskRunner;
  // FML_DCHECK(RasterTaskRunner->RunsTasksOnCurrentThread());
  // Get callback on raster thread and jump back to platform thread.
  fml::WeakPtr<FlutterViewController> weakViewController = [viewController getWeakPtr];
  platformTaskRunner->PostTask([weakViewController]() {
...
- (void)installFirstFrameCallback {
  ...
  if (_engine.get().enableEmbedderAPI) {
    // TODO(cyanglaz): embedder api, first frame callback
    [_engine.get() setNextFrameCallback:FirstFrameCallback userData:(__bridge void*)self];
    return;
  }
...
- (void)createTouchRateCorrectionVSyncClientIfNeeded {
  ...
  // TODO(cyanglaz): embedder api, enable this vsync client in embedder api
  if (!_engine.get().enableEmbedderAPI) {
...
#endif
    bool successfullyWaitForFirstFrame;
    if (_engine.get().enableEmbedderAPI) {
      successfullyWaitForFirstFrame = [_engine.get() waitForFirstFrame:waitTime];
    } else {
      flutter::Shell& shell = [_engine.get() shell];
      successfullyWaitForFirstFrame =
          shell.WaitForFirstFrame(waitTime).code() == fml::StatusCode::kDeadlineExceeded;
    }
    if (!successfullyWaitForFirstFrame) {
