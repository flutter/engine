...
TEST(EncodableValueTest, Map) {
  EncodableMap encodables = {
      ...
      {EncodableValue(1), EncodableValue(10000ll)},
  ...
  EXPECT_EQ(map_value[EncodableValue(1)].LongValue(), 10000ll);
...
TEST(EncodableValueTest, EmptyTypeConstructor) {
  ...
  EXPECT_EQ(EncodableValue(EncodableValue::Type::kLong).LongValue(), 0ll);
...
TEST(EncodableValueTest, Comparison) {
  EncodableList values = {
      ...
      // LongList
      EncodableValue(std::vector<int64_t>{0, 1ll}),
      EncodableValue(std::vector<int64_t>{0, 100ll}),
      // DoubleList
      EncodableValue(std::vector<int64_t>{0, 1ll}),
      EncodableValue(std::vector<int64_t>{0, 100ll}),
      // List
