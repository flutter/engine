...
fuchsia::ui::gfx::mat4 GetNodeTransform(const FlutterSemanticsNode* node) {
  SkMatrix44 node_transform(SkMatrix::MakeAll(
      node->transform.scaleX, node->transform.skewX, node->transform.transX,
      node->transform.skewY, node->transform.scaleY, node->transform.transY,
      node->transform.pers0, node->transform.pers1, node->transform.pers2));
  fuchsia::ui::gfx::mat4 value;
  node_transform.asColMajorf(value.matrix.data());
  return value;
}
...
void ScenicAccessibilityBridge::AddSemanticsNodeUpdate(
  ...
  FML_DCHECK(node->id >= 0);  // -1 is the only valid negative value
  ...
  if (this_node_size >= kMaxMessageSize) {
    FML_LOG(ERROR) << "Semantics node with ID " << node->id
