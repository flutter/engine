...
void AccessibilityBridge::UpdateScreenRects(int32_t node_id,
  ...
  for (SkVector4 point : points) {
    node.screen_rect.setLTRB(std::min(node.screen_rect.left(), point.fData[0]),
                         std::min(node.screen_rect.top(), point.fData[1]),
                         std::min(node.screen_rect.right(), point.fData[0]),
                         std::min(node.screen_rect.bottom(), point.fData[1]));
  }
...
void AccessibilityBridge::HitTest(
  ...
  FML_LOG(ERROR) << "Going to try to get node for point " << local_point.x << "," << local_point.y;
  GetHitNode(kRootNodeId, local_point.x, local_point.y, &hit);
  FML_LOG(ERROR) << "Point " << local_point.x << "," << local_point.y <<" got hit: " << hit.node_id();
...
void AccessibilityBridge::GetHitNode(
  ...
  if (node.flags &
      ...
      x < node.screen_rect.left() ||                                   //
      x >= node.screen_rect.right() ||                                 //
      y < node.screen_rect.top() ||                                    //
      y >= node.screen_rect.bottom()) {
    return;
  }
