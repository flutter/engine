// Adapted from $FUCHSIA_DIR/sdk/lib/sys/cpp/testing/component_interceptor.cc
...
constexpr char kAutogenCmxPath[] = "fuchsia-pkg://example.com/fake_pkg#autogenerated_manifest.cmx";
}  // namespace

ComponentInterceptor::ComponentInterceptor(fuchsia::sys::LoaderPtr fallback_loader,
                                           async_dispatcher_t* dispatcher)
    : fallback_loader_(std::move(fallback_loader)), dispatcher_(dispatcher) {
  loader_svc_ =
      std::make_shared<vfs::Service>([this](zx::channel h, async_dispatcher_t* /*unused*/) mutable {
        loader_bindings_.AddBinding(
            this, fidl::InterfaceRequest<fuchsia::sys::Loader>(std::move(h)), dispatcher_);
      });
}
...
ComponentInterceptor ComponentInterceptor::CreateWithEnvironmentLoader(
  ...
  sp->ConnectToService(fuchsia::sys::Loader::Name_, fallback_loader.NewRequest().TakeChannel());

  return ComponentInterceptor(std::move(fallback_loader), dispatcher);
}

std::unique_ptr<EnvironmentServices> ComponentInterceptor::MakeEnvironmentServices(
...
bool ComponentInterceptor::InterceptURL(std::string component_url, std::string extra_cmx_contents,
  ...
  info.pkg_dir->AddEntry(kAutogenPkgDirManifestPath,
                         std::make_unique<vfs::PseudoFile>(
                             cmx_str.length(), [cmx_str = std::move(cmx_str)](
...
void ComponentInterceptor::StartComponent(
    fuchsia::sys::Package /*unused*/, fuchsia::sys::StartupInfo startup_info,
  ...
  handler(std::move(startup_info),
          std::make_unique<InterceptedComponent>(std::move(controller), dispatcher_));
...
InterceptedComponent::InterceptedComponent(
    ...
    async_dispatcher_t* dispatcher)
    : binding_(this), termination_reason_(TerminationReason::EXITED), exit_code_(ZX_OK) {
...
void InterceptedComponent::Kill() { KillImpl(); }

void InterceptedComponent::KillImpl() {
  if (on_kill_) {
    on_kill_();
  }
