...
fuchsia::feedback::CrashReport BuildCrashReport(
  ...
  if (delimiter_pos == std::string::npos) {
    FX_LOG(ERROR) << "error parsing Dart exception: expected format "
  ...
  if (!SetStackTrace(stack_trace, &dart_report)) {
    FX_LOG(ERROR) << "Failed to convert Dart stack trace to VMO";
  }
...
void HandleException(std::shared_ptr<::sys::ServiceDirectory> services,
  ...
  crash_reporter->File(
      ...
      [](fuchsia::feedback::CrashReporter_File_Result result) {
        if (result.is_err()) {
          FX_LOG(ERROR) << "Failed to report Dart exception: " << result.err()
