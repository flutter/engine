...
public class SplashShadowResources {
  ...
  public Drawable getDrawableForDensity(int id, int density, Resources.Theme theme) throws Exception {
    if (id == SPLASH_DRAWABLE_ID) {
      return new ColorDrawable(Color.BLUE);
    }

    if (id == THEMED_SPLASH_DRAWABLE_ID) {
      // We pretend the drawable contains theme references. It can't be parsed without the app theme.
      if (theme == null) {
        throw new Resources.NotFoundException("Cannot parse drawable due to missing theme references.");
      }
