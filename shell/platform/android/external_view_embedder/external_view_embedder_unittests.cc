...
//   auto view_params =
//       std::make_unique<EmbeddedViewParams>(matrix, SkSize::Make(30, 40),
//       stack);

//   auto view_id = 0;
//   embedder->PrerollCompositeEmbeddedView(view_id, std::move(view_params));
//   ASSERT_EQ(SkRect::MakeXYWH(15, 30, 45, 60),
...
//   ASSERT_EQ(SkRect::MakeXYWH(75, 90, 105, 120),
//   embedder->GetViewRect(view_id));
// }
...
//       [gr_context, window, frame_size, framebuffer_info, jni_mock]() {
...
//         CreateGPUSurface(gr_context.get()))
...
//     EXPECT_CALL(*jni_mock, FlutterViewEndFrame());
//     embedder->EndFrame(/*should_resubmit_frame=*/false,
...
//     // This simulates Flutter UI that doesn't intersect with the Android
//     view. canvas_1->DrawRect(SkRect::MakeXYWH(0, 0, 50, 50), rect_paint);
...
//     embedder->SubmitFlutterView(kImplicitViewId, gr_context.get(), nullptr,
//                                 std::move(surface_frame));
//     // Doesn't submit frame if there aren't Android views in the previous
//     frame. EXPECT_FALSE(did_submit_frame);
//     // Resubmits frame.
//     auto postpreroll_result =
//     embedder->PostPrerollAction(raster_thread_merger);
...
//     MutatorsStack stack;
//     embedder->PrerollCompositeEmbeddedView(
//         0, std::make_unique<EmbeddedViewParams>(matrix, SkSize::Make(100,
//         100),
//                                                 stack));
//     // The JNI call to display the Android view.
//     EXPECT_CALL(*jni_mock, FlutterViewOnDisplayPlatformView(
...
//     MutatorsStack stack;
//     embedder->PrerollCompositeEmbeddedView(
//         1, std::make_unique<EmbeddedViewParams>(matrix, SkSize::Make(100,
//         100),
//                                                 stack));
//     // The JNI call to display the Android view.
//     EXPECT_CALL(*jni_mock, FlutterViewOnDisplayPlatformView(
...
//   // Since we will compute the intersection for each android view in turn,
//   and
...
//     embedder->PrerollCompositeEmbeddedView(
//         0, std::make_unique<EmbeddedViewParams>(matrix, SkSize::Make(200,
//         200),
//                                                 stack));
//     EXPECT_CALL(*jni_mock, FlutterViewOnDisplayPlatformView(0, 0, 0, 200,
//     200,
//                                                             300, 300,
//                                                             stack));
//   }
...
//         100),
//                                                 stack));
//     EXPECT_CALL(*jni_mock, FlutterViewOnDisplayPlatformView(1, 0, 0, 100,
//     100,
//                                                             150, 150,
//                                                             stack));
//   }
//   // This simulates Flutter UI that intersects with the first and second
//   Android
//   // views.
//   embedder->CompositeEmbeddedView(1)->DrawRect(SkRect::MakeXYWH(25, 25, 80,
//   50),
//                                                rect_paint);
...
//   EXPECT_CALL(*jni_mock, FlutterViewDisplayOverlaySurface(1, 25, 25, 80,
//   150))
//       .Times(2);
...
//   // While on the raster thread, don't make JNI calls as these methods can
//   only
...
//   // The shell may disable the thread merger during
//   `OnPlatformViewDestroyed`. raster_thread_merger->Disable();
