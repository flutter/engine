...
FlutterKeyboardKeyEmbedderHandler::FlutterKeyboardKeyEmbedderHandler(
...
FlutterKeyboardKeyEmbedderHandler::~FlutterKeyboardKeyEmbedderHandler() = default;
...
uint64_t FlutterKeyboardKeyEmbedderHandler::getPhysicalKey(int scancode, bool extended) {
...
uint64_t FlutterKeyboardKeyEmbedderHandler::getLogicalKey(int key, bool extended, int scancode) {
  ...
  auto numpadIt = scanCodeToLogicalMap_.find(normalizeScancode(scancode, extended));
...
void FlutterKeyboardKeyEmbedderHandler::cacheUtf8String(char32_t character) {
  if (character == 0) {
...
void FlutterKeyboardKeyEmbedderHandler::KeyboardHook(
    int key,
    int scancode,
    int action,
    char32_t character,
  ...
  PendingResponse pending{
    .callback = [this, callback = std::move(callback)]
        (bool handled, uint64_t reponse_id) {
  ...
  sendEvent_(key_data,
             FlutterKeyboardKeyEmbedderHandler::HandleResponse,
             reinterpret_cast<void*>(pending_responses_[response_id].get()));
}

void FlutterKeyboardKeyEmbedderHandler::HandleResponse(
