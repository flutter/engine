...
class MockPlatformHandler : public PlatformHandler {
 public:
  ...
  MOCK_METHOD4(QuitApplication, void(std::optional<HWND> hwnd, std::optional<WPARAM> wparam, std::optional<LPARAM> lparam, UINT exit_code));
...
TEST_F(PlatformHandlerTest, SystemExitApplicationRequired) {
  ...
  ON_CALL(platform_handler, QuitApplication)
      .WillByDefault([&exit_code](std::optional<HWND> hwnd, std::optional<WPARAM> wparam, std::optional<LPARAM> lparam, UINT ec) { exit_code = ec; });
