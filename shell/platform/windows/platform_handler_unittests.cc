...
static constexpr char kSystemExitApplicationRequiredMessage[] =
    "{\"method\":\"System.exitApplication\",\"args\":{\"type\":\"required\",\"exitCode\":1}}";
static constexpr char kSystemExitApplicationCancelableMessage[] =
    "{\"method\":\"System.exitApplication\",\"args\":{\"type\":\"cancelable\",\"exitCode\":2}}";
static constexpr char kExitResponseCancelMessage[] =
    "[{\"response\":\"cancel\"}]";
static constexpr char kExitResponseExitMessage[] =
    "[{\"response\":\"exit\"}]";
...
TEST_F(PlatformHandlerTest, SystemExitApplicationRequired) {
  ...
  TestBinaryMessenger messenger([](const std::string& channel, const uint8_t* message, size_t size, BinaryReply reply) {
  } );
  MockPlatformHandler platform_handler(&messenger, engine());

  ON_CALL(platform_handler, QuitApplication).WillByDefault([&exit_code](int ec) {exit_code = ec;});
  EXPECT_CALL(platform_handler, QuitApplication).Times(1);

  std::string result = SimulatePlatformMessage(&messenger, kSystemExitApplicationRequiredMessage);
...
TEST_F(PlatformHandlerTest, SystemExitApplicationCancelableCancel) {
  ...
  TestBinaryMessenger messenger([&called_cancel](const std::string& channel, const uint8_t* message, size_t size, BinaryReply reply) {
    reply(reinterpret_cast<const uint8_t*>(kExitResponseCancelMessage), sizeof(kExitResponseCancelMessage));
    called_cancel = true;
  } );
  MockPlatformHandler platform_handler(&messenger, engine());
  ...
  std::string result = SimulatePlatformMessage(&messenger, kSystemExitApplicationCancelableMessage);
...
TEST_F(PlatformHandlerTest, SystemExitApplicationCancelableExit) {
  ...
  TestBinaryMessenger messenger([&called_cancel](const std::string& channel, const uint8_t* message, size_t size, BinaryReply reply) {
    reply(reinterpret_cast<const uint8_t*>(kExitResponseExitMessage), sizeof(kExitResponseExitMessage));
    called_cancel = true;
  } );
  MockPlatformHandler platform_handler(&messenger, engine());
