...
#include "include/flutter/win32_window_pub.h"
...
Win32WindowPub::Win32WindowPub() {
  ...
  current_dpi_ = pubdpi::GetDpiForHWND(nullptr);
}
...
void Win32WindowPub::InitializeChild(const char* title,
                                  unsigned int width,
                                  unsigned int height) {
  Destroy();
...
std::wstring Win32WindowPub::NarrowToWide(const char* source) {
...
LRESULT CALLBACK Win32WindowPub::WndProc(HWND const window,
                                      UINT const message,
                                      WPARAM const wparam,
  ...
  if (message == WM_NCCREATE) {
    ...
    auto that = static_cast<Win32WindowPub*>(cs->lpCreateParams);
    that->window_handle_ = window;
  } else if (Win32WindowPub* that = GetThisFromHandle(window)) {
...
void Win32WindowPub::TrackMouseLeaveEvent(HWND hwnd) {
...
LRESULT
Win32WindowPub::MessageHandler(HWND hwnd,
                            UINT const message,
                            WPARAM const wparam,
                            LPARAM const lparam) noexcept {
  int xPos = 0, yPos = 0;
  ...
  auto window =
      reinterpret_cast<Win32WindowPub*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
...
UINT Win32WindowPub::GetCurrentDPI() {
  return current_dpi_;
}

UINT Win32WindowPub::GetCurrentWidth() {
  return current_width_;
}

UINT Win32WindowPub::GetCurrentHeight() {
  return current_height_;
}

HWND Win32WindowPub::GetWindowHandle() {
...
void Win32WindowPub::HandleResize(UINT width, UINT height) {
  current_width_ = width;
  current_height_ = height;
  OnResize(width, height);
}

Win32WindowPub* Win32WindowPub::GetThisFromHandle(HWND const window) noexcept {
  return reinterpret_cast<Win32WindowPub*>(
