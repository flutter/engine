...
class Win32WindowPub {
 public:
  Win32WindowPub();
  virtual ~Win32WindowPub();
 ...
 protected:
  ...
  MouseStatePub GetMouseStatePub() { return mouse_state_; }

  // Resets the mouse state to its default values.
  void ResetMouseState() { mouse_state_ = MouseStatePub(); }
 ...
 private:
  ...
  static Win32WindowPub* GetThisFromHandle(HWND const window) noexcept;
  int current_dpi_ = 0;
