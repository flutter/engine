...
EmbedderEngine::EmbedderEngine(
    ThreadHost thread_host, blink::TaskRunners task_runners,
    blink::Settings settings,
    Shell::CreateCallback<PlatformView> on_create_platform_view,
    Shell::CreateCallback<Rasterizer> on_create_rasterizer)
    : thread_host_(std::move(thread_host)),
      shell_(Shell::Create(std::move(task_runners), std::move(settings),
...
bool EmbedderEngine::IsValid() const { return is_valid_; }
...
bool EmbedderEngine::Run(RunConfiguration run_configuration) {
  ...
  shell_->GetTaskRunners().GetUITaskRunner()->PostTask([
    engine = shell_->GetEngine(),          // engine
