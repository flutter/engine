...
#include "flutter/shell/platform/embedder/macos_compositor.h"
...
MacOSCompositor::MacOSCompositor(SkISize surface_size,
                                sk_sp<GrDirectContext> context)
    : surface_size_(surface_size), context_(context) {
  FML_CHECK(!surface_size_.isEmpty()) << "Surface size must not be empty";
  FML_CHECK(context_);

  // Set to Software Render for now.
  type_ = MacOSCompositor::RenderTargetType::kSoftwareBuffer;
}

MacOSCompositor::~MacOSCompositor() = default;

void MacOSCompositor::SetRenderTargetType(RenderTargetType type) {
  type_ = type;
}
...
bool MacOSCompositor::CollectBackingStore(
...
bool MacOSCompositor::UpdateOffscrenComposition(
...
sk_sp<SkImage> MacOSCompositor::GetLastComposition() {
  return last_composition_;
}

bool MacOSCompositor::Present(const FlutterLayer** layers,
                                     size_t layers_count) {
...
bool MacOSCompositor::CreateFramebufferRenderSurface(
    const FlutterBackingStoreConfig* config,
    FlutterBackingStore* backing_store_out) {
...
bool MacOSCompositor::CreateTextureRenderSurface(
    const FlutterBackingStoreConfig* config,
    FlutterBackingStore* backing_store_out) {
  const auto image_info =
      SkImageInfo::MakeN32Premul(config->size.width, config->size.height);
...
bool MacOSCompositor::CreateSoftwareRenderSurface(
    const FlutterBackingStoreConfig* config,
    FlutterBackingStore* backing_store_out) {
