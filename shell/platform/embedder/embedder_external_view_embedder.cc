...
void EmbedderExternalViewEmbedder::BeginFrame(
  ...
  // |ExternalViewEmbedder|
  void EmbedderExternalViewEmbedder::PrepareView(
    ...
    pending_views_[kRootViewIdentifier] =
        std::make_unique<EmbedderExternalView>(pending_frame_size_,
  ...
  // |ExternalViewEmbedder|
  DlCanvas* EmbedderExternalViewEmbedder::CompositeEmbeddedView(
      int64_t view_id) {
    auto vid = EmbedderExternalView::ViewIdentifier(view_id);
  ...
  void EmbedderExternalViewEmbedder::SubmitView(
    ...
    for (const auto& pending_key : pending_keys) {
      ...
      // If the external view does not have engine rendered contents, it makes
      // no sense to ask to embedder to create a render target for us as we
      // don't intend to render into it and ask the embedder for presentation
      // anyway. Save some memory.
      ...
      // flow to the embedder makes the engine susceptible to having the
      // embedder trample on the OpenGL context. Before any Skia operations are
      // performed, the context must be reset.
      //
      // @warning: Embedder may trample on our OpenGL context here.
    ...
    // @warning: Embedder may trample on our OpenGL context here.
    {
      ...
      for (const auto& view_id : composition_order_) {
        // If the external view has a platform view, ask the emebdder to place
        // it before the Flutter rendered contents for that interleaving level.
        ...
        // platform view.
        if (external_view->HasEngineRenderedContents()) {
          const auto& exteral_render_target =
              matched_render_targets.at(view_id);
          const auto& external_view = pending_views_.at(view_id);
          auto rect_list =
