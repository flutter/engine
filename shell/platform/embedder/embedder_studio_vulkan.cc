...
EmbedderStudioVulkan::EmbedderStudioVulkan(
    uint32_t version,
    VkInstance instance,
...
const vulkan::VulkanProcTable& EmbedderStudioVulkan::vk() {
  return *vk_;
}

// |GPUSurfaceVulkanDelegate|
FlutterVulkanImage EmbedderStudioVulkan::AcquireImage(const SkISize& size) {
  return vulkan_dispatch_table_.get_next_image(size);
}

// |GPUSurfaceVulkanDelegate|
bool EmbedderStudioVulkan::PresentImage(VkImage image, VkFormat format) {
...
// |EmbedderStudio|
sk_sp<GrDirectContext> EmbedderStudioVulkan::CreateResourceContext() const {
  return resource_context_;
}

sk_sp<GrDirectContext> EmbedderStudioVulkan::CreateGrContext(
    VkInstance instance,
    uint32_t version,
    size_t instance_extension_count,
    const char** instance_extensions,
