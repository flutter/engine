...
EmbedderVsyncWaiter::EmbedderVsyncWaiter(const VsyncCallback& vsync_callback,
                                         flutter::TaskRunners task_runners)
    : VsyncWaiter(std::move(task_runners)), vsync_callback_(vsync_callback) {
  FML_DCHECK(vsync_callback_);
}

EmbedderVsyncWaiter::~EmbedderVsyncWaiter() = default;

void EmbedderVsyncWaiter::AwaitVSync() {
  auto* weak_waiter = new std::weak_ptr<VsyncWaiter>(shared_from_this());
  vsync_callback_(reinterpret_cast<intptr_t>(weak_waiter));
}

bool EmbedderVsyncWaiter::OnEmbedderVsync(intptr_t baton,
