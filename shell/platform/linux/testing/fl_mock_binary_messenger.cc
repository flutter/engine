...
static MockErrorChannel* mock_error_channel_new(
...
static void fl_mock_binary_messenger_send_on_channel(
  ...
  if (mock_channel != nullptr) {
    ...
    if (!fl_method_codec_decode_method_call(mock_method_channel->codec, message,
     ...
     g_task_return_new_error(task, fl_binary_messenger_codec_error_quark(),mock_error_channel->code, "%s", mock_error_channel->message);
     return;
  }
...
static void fl_mock_binary_messenger_init(FlMockBinaryMessenger* self) {
  ...
  self->mock_error_channels =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                            (GDestroyNotify)mock_error_channel_free);
}
...
void fl_mock_binary_messenger_set_error_channel(
    FlMockBinaryMessenger* self,
    const gchar* channel,gint code,
						const gchar *message) {
  g_return_if_fail(FL_IS_MOCK_BINARY_MESSENGER(self));
