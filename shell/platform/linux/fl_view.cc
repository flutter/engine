...
static gboolean fl_view_send_pointer_button_event(FlView* self,
  ...
  fl_engine_send_mouse_pointer_event(
      self->engine, phase, event->time * kMicrosecondsPerMillisecond, event->x,
      event->y, 0, 0, self->button_state);
  return TRUE;
}
...
static gboolean fl_view_scroll_event(GtkWidget* widget, GdkEventScroll* event) {
  ...
  fl_engine_send_mouse_pointer_event(
      ...
      event->time * kMicrosecondsPerMillisecond, event->x, event->y,
      scroll_delta_x, scroll_delta_y, self->button_state);
...
static gboolean fl_view_motion_notify_event(GtkWidget* widget,
  ...
  fl_engine_send_mouse_pointer_event(
      self->engine, self->button_state != 0 ? kMove : kHover,
      event->time * kMicrosecondsPerMillisecond, event->x, event->y, 0, 0,
      self->button_state);
