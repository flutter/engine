...
static uint64_t pressed_logical_for_physical(GHashTable* pressing_records, uint64_t physical) {
  uint64_t* logical_pointer = reinterpret_cast<uint64_t*>(g_hash_table_lookup(pressing_records, &physical));
  if (logical_pointer == NULL)
    return 0;
...
static int decrease_pressed_logical_count(GHashTable* pressed_logicals, uint64_t logical) {
  gpointer count_pointer;
  g_hash_table_lookup_extended(pressed_logicals, &logical, NULL, &count_pointer);
  uint64_t count = *(uint64_t*)count_pointer;
  if (count == 1) {
    *(uint64_t*)count_pointer = count - 1;
  } else {
    g_hash_table_remove(pressed_logicals, &logical);
  }
  return count;
}
...
static int assign_pressed_logical(GHashTable* pressing_records,
  ...
  uint64_t* logical_pointer;
  int* logical_count;
  gboolean found_logical = g_hash_table_lookup_extended(
      pressed_logicals,
      &logical,
      reinterpret_cast<gpointer*>(&logical_pointer),
      reinterpret_cast<gpointer*>(&logical_count));
  if (!found_logical) {
    logical_pointer = reinterpret_cast<uint64_t*>(g_malloc(sizeof(logical)));
    *logical_pointer = logical;
    logical_count = reinterpret_cast<int*>(g_malloc(sizeof(int)));
    *logical_count = 1;
    g_hash_table_insert(pressed_logicals, logical_pointer, logical_count);
  } else {
    *logical_count += 1;
  }
  printf("insert %lu\n", physical);
  g_hash_table_insert(pressing_records, &physical, logical_pointer);
...
size_t fl_keyboard_manager_convert_key_event(FlKeyboardManager* self,
  ...
  if (last_logical_record != next_logical_record) {
    ...
    if (next_logical_record == 0) {
      g_hash_table_remove(self->pressing_records, reinterpret_cast<gpointer>(&physical_key));
      printf("removed %lu\n", physical_key);
    }
    printf("aftersize %d\n", g_hash_table_size(self->pressing_records));
    fflush(stdout);
...
FlKeyboardManager* fl_keyboard_manager_new(FlView* view) {
  ...
  self->pressing_records = g_hash_table_new_full(g_int64_hash, g_int64_equal, g_free, NULL);
  self->pressed_logicals = g_hash_table_new_full(g_int64_hash, g_int64_equal, g_free, NULL);
