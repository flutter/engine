...
TEST(FlKeyEmbedderResponderTest, ReleaseShiftKeyBetweenDigitKeyEvents) {
  ...
  state = static_cast<GdkModifierType>(GDK_SHIFT_MASK);
...
TEST(FlKeyEmbedderResponderTest, SynthesizeForDesyncPressingStateOnSelfEvents) {
  ...
  GdkModifierType state = static_cast<GdkModifierType>(GDK_CONTROL_MASK);
  ...
  state = static_cast<GdkModifierType>(GDK_CONTROL_MASK);
  fl_key_responder_handle_event(
      responder,
