...
class CallRecord {
 public:
  ...
  CallRecord(CallRecord&& origin)
      : responder(origin.responder), event(origin.event), callback(std::move(origin.callback)) {
...
static FlKeyMockResponder* fl_key_mock_responder_new(std::vector<CallRecord>* call_records,
                                                     int delegate_id) {
