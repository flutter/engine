...
static uint64_t event_to_physical_key(const GdkEventKey* event,
                                      GHashTable* table) {
  gpointer record =
      g_hash_table_lookup(table, GUINT_TO_POINTER(event->hardware_keycode));
  if (record != nullptr) {
...
static uint64_t pressed_logical_for_physical(GHashTable* pressing_records,
                                             uint64_t physical) {
  return gpointerToUint64(
      g_hash_table_lookup(pressing_records, uint64ToGpointer(physical)));
}
...
static void fl_key_embedder_responder_handle_event(
  ...
  uint64_t last_logical_record =
      pressed_logical_for_physical(self->pressing_records, physical_key);
