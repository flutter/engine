...
std::unique_ptr<SurfaceFrame> GPUSurfaceVulkanImpeller::AcquireFrame(
  ...
  FML_LOG(ERROR) << "acquriring SurfaceVK";
  std::unique_ptr<impeller::Surface> surface = context_vk.AcquireSurface(0);

  auto swap_callback = [weak = weak_factory_.GetWeakPtr()]() -> bool {
    if (weak) {
      // TODO present vulkan image.
      FML_LOG(ERROR) << "presenting SurfaceVK";
    }
    return true;
  };
