...
class RTreeBoundsAccumulator final : public virtual BoundsAccumulator {
 public:
  RTreeBoundsAccumulator() : rects_(std::make_unique<std::vector<SkRect>>()) {}
  void accumulate(const SkRect& r) override;
  void save() override;
  void restore() override;
  ...
  std::unique_ptr<std::vector<SkRect>> rects() {
    FML_DCHECK(saved_offsets_.empty());
    return std::move(rects_);
  }

 private:
  std::unique_ptr<std::vector<SkRect>> rects_;
  std::vector<size_t> saved_offsets_;
};

class CombineBoundsAccumulator final : public virtual BoundsAccumulator {
 public:
  void push_back(BoundsAccumulator* accumulator) {
  ...
  bool restore(
    ...
    bool result = true;
    for (auto accumulator : accumulators_) {
      result = accumulator->restore(map, clip) && result;
    }
    return result;
  }

 private:
  std::vector<BoundsAccumulator*> accumulators_;
};
