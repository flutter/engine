...
TEST(DisplayListRTree, NullSearchResultVector) {
  ...
  EXPECT_DEATH_IF_SUPPORTED(tree.search(DlFRect::MakeLTRB(0, 0, 1, 1), nullptr),
                            "results != nullptr");
}
#endif
...
TEST(DisplayListRTree, NullRectListZeroCount) {
  ...
  auto huge = DlFRect::MakeLTRB(-1e6, -1e6, 1e6, 1e6);
...
TEST(DisplayListRTree, ManySizes) {
  ...
  for (int N = 0; N <= kMaxN; N++) {
    ...
    EXPECT_EQ(tree.bounds(-1), DlFRect()) << desc;
    EXPECT_EQ(tree.id(N), -1) << desc;
    EXPECT_EQ(tree.bounds(N), DlFRect()) << desc;
    results.clear();
    tree.search(DlFRect(), &results);
    EXPECT_EQ(results.size(), 0u) << desc;
    results.clear();
    tree.search(DlFRect::MakeLTRB(2, 2, 8, 8), &results);
    if (N == 0) {
      EXPECT_EQ(results.size(), 0u) << desc;
    } else {
      ...
      for (int i = 1; i < N; i++) {
        results.clear();
        auto query = DlFRect::MakeXYWH(i * 20 + 2, i * 20 + 2, 6, 6);
...
TEST(DisplayListRTree, HugeSize) {
  ...
  EXPECT_EQ(tree.bounds(-1), DlFRect());
  EXPECT_EQ(tree.id(N), -1);
  EXPECT_EQ(tree.bounds(N), DlFRect());
  std::vector<int> results;
  tree.search(DlFRect(), &results);
  EXPECT_EQ(results.size(), 0u);
  for (int i = 0; i < N; i++) {
    results.clear();
    tree.search(DlFRect::MakeXYWH(i * 20 + 2, i * 20 + 2, 6, 6), &results);
...
TEST(DisplayListRTree, Grid) {
  ...
  for (int r = 0; r < ROWS; r++) {
    ...
    for (int c = 0; c < COLS; c++) {
      int x = c * 20 + 5;
      int i = r * COLS + c;
      rects[i] = DlFRect::MakeXYWH(x, y, 10, 10);
      ids[i] = i + 42;
    }
  }
  DlRTree tree(rects, N, ids);
  ...
  for (int r = 0; r < ROWS; r++) {
    ...
    for (int c = 0; c < COLS; c++) {
      ...
      results.clear();
      auto query = DlFRect::MakeXYWH(x + 2, y + 2, 6, 6);
      tree.search(query, &results);
      EXPECT_EQ(results.size(), 1u) << desc;
      EXPECT_EQ(results[0], i) << desc;
      EXPECT_EQ(tree.id(results[0]), ids[i]) << desc;
  ...
  for (int r = 1; r < ROWS; r++) {
    ...
    for (int c = 1; c < COLS; c++) {
      ...
      auto desc =
          "row " + std::to_string(r + 1) + ", col " + std::to_string(c + 1);
      results.clear();
      auto query = DlFRect::MakeXYWH(x - 8, y - 8, 6, 6);
      tree.search(query, &results);
      EXPECT_EQ(results.size(), 0u) << desc;
      auto list = tree.searchAndConsolidateRects(query);
  ...
  for (int r = 1; r < ROWS; r++) {
    ...
    for (int c = 1; c < COLS; c++) {
      ...
      auto desc =
          "row " + std::to_string(r + 1) + ", col " + std::to_string(c + 1);
      results.clear();
      auto query = DlFRect::MakeXYWH(x - 11, y - 11, 12, 12);
      tree.search(query, &results);
...
TEST(DisplayListRTree, OverlappingRects) {
  ...
  for (int r = 0; r < 3; r++) {
    ...
    for (int c = 0; c < 3; c++) {
      int x = 15 + 20 * c;
      auto query = DlFRect::MakeLTRB(x - 1, y - 1, x + 1, y + 1);
      auto list = tree.searchAndConsolidateRects(query);
      EXPECT_EQ(list.size(), 1u);
  ...
  for (int r = 0; r < 3; r++) {
    ...
    int y = 15 + 20 * r;
    int x = 15 + 20 * c;
    auto query = DlFRect::MakeLTRB(x - 6, y - 1, x + 6, y + 1);
    auto list = tree.searchAndConsolidateRects(query);
    EXPECT_EQ(list.size(), 1u);
    EXPECT_EQ(list.front(), DlFRect::MakeLTRB(x - 35, y - 15, x + 35, y + 15));
  }
  ...
  for (int c = 0; c < 3; c++) {
    ...
    int x = 15 + 20 * c;
    int y = 15 + 20 * r;
    auto query = DlFRect::MakeLTRB(x - 1, y - 6, x + 1, y + 6);
    auto list = tree.searchAndConsolidateRects(query);
    EXPECT_EQ(list.size(), 1u);
    EXPECT_EQ(list.front(), DlFRect::MakeLTRB(x - 15, y - 35, x + 15, y + 35));
  }
  // Finally intersecting all 9 rects
  auto query = DlFRect::MakeLTRB(35 - 6, 35 - 6, 35 + 6, 35 + 6);
  auto list = tree.searchAndConsolidateRects(query);
  EXPECT_EQ(list.size(), 1u);
  EXPECT_EQ(list.front(), DlFRect::MakeLTRB(0, 0, 70, 70));
}

TEST(DisplayListRTree, Region) {
  DlFRect rect[9];
  for (int i = 0; i < 9; i++) {
    rect[i] = DlFRect::MakeXYWH(i * 10, i * 10, 20, 20);
  }
