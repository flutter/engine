...
DlRTree::DlRTree(const DlFRect rects[],
                 int N,
                 const int ids[],
                 bool p(int),
                 int invalid_id)
...
std::list<DlFRect> DlRTree::searchAndConsolidateRects(const DlFRect& query,
                                                      bool deband) const {
...
const DlFRect& DlRTree::bounds() const {
  if (!nodes_.empty()) {
