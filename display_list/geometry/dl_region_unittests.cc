...
TEST(DisplayListRegion, SingleRectangle) {
  DlRegion region(std::vector<DlIRect>({DlIRect::MakeLTRB(10, 10, 50, 50)}));
...
void CheckEquality(const DlRegion& dl_region, const SkRegion& sk_region) {
  EXPECT_EQ(dl_region.bounds(), DlIRect::MakeBounds(sk_region.getBounds()));
  ...
  while (!iterator.done()) {
    skia_rects.push_back(DlIRect::MakeBounds(iterator.rect()));
    iterator.next();
  }
  ...
  for (const auto& settings : all_settings) {
    for (const auto iterations_1 : iterations) {
      for (const auto iterations_2 : iterations) {
        ...
        std::vector<DlIRect> dl_rects_in1;
        std::vector<DlIRect> dl_rects_in2;
        std::vector<SkIRect> sk_rects_in1;
        std::vector<SkIRect> sk_rects_in2;

        for (size_t i = 0; i < iterations_1; ++i) {
          SkIRect rect =
              SkIRect::MakeXYWH(pos(rng), pos(rng), size(rng), size(rng));
          sk_rects_in1.push_back(rect);
          dl_rects_in1.push_back(DlIRect::MakeBounds(rect));
        }

        for (size_t i = 0; i < iterations_2; ++i) {
          SkIRect rect =
              SkIRect::MakeXYWH(pos(rng), pos(rng), size(rng), size(rng));
          sk_rects_in2.push_back(rect);
          dl_rects_in2.push_back(DlIRect::MakeBounds(rect));
        }

        DlRegion region1(dl_rects_in1);
        sk_region1.setRects(sk_rects_in1.data(), sk_rects_in1.size());
        CheckEquality(region1, sk_region1);

        DlRegion region2(dl_rects_in2);
        sk_region2.setRects(sk_rects_in2.data(), sk_rects_in2.size());
        ...
        {
          auto iterator = SkRegion::Iterator(sk_region1);
          while (!iterator.done()) {
            SkIRect r = iterator.rect();
            EXPECT_EQ(region1.intersects(DlIRect::MakeBounds(r)),
                      sk_region1.intersects(r));
            iterator.next();
          }
        }
