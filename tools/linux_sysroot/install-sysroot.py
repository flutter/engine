...
DEFAULT_SYSROOTS_PATH = os.path.join(os.path.relpath(SCRIPT_DIR, SRC_DIR), 'sysroots.json')
...
def main(args):
  ...
  parser.add_option('--sysroots-json-path', help='The location of sysroots.json file')
  parser.add_option('--arch', help='Sysroot architecture: %s' % ', '.join(VALID_ARCHS))
  parser.add_option(
      '--all',
      action='store_true',
  ...
  parser.add_option('--print-key', help='Print the hash of the sysroot for the given arch.')
  ...
  if options.print_key:
    ...
    print(
        GetSysrootDict(
            sysroots_json_path, DEFAULT_TARGET_PLATFORM, ARCH_TRANSLATIONS.get(arch, arch)
        )['Key']
    )
    return 0
  if options.arch:
    InstallSysroot(
        sysroots_json_path, DEFAULT_TARGET_PLATFORM,
...
def InstallSysroot(sysroots_json_path, target_platform, target_arch):
  sysroot_dict = GetSysrootDict(sysroots_json_path, target_platform, target_arch)
