...
bool IsCollapsed(const AXNode* node) {
  return node && node->data().HasState(ax::State::kCollapsed);
}
...
struct AXTreeUpdateState {
  ...
  bool DoesPendingNodeRequireInit(AXNode::AXID node_id) const {
    if (AXTreePendingStructureStatus::kComputing != pending_update_status) {
      FML_LOG(ERROR) << "This method should only be called while computing "
                        "pending changes, "
...
SkRect AXTree::RelativeToTreeBoundsInternal(const AXNode* node,
                                            SkRect bounds,
                                            bool* offscreen,
                                            bool clip_bounds,
  ...
  if (bounds.width() == 0 && bounds.height() == 0) {
    ...
    if (bounds.isEmpty() && !GetTreeUpdateInProgressState() &&
      ...
      for (size_t i = 0; i < node->children().size(); i++) {
        ax::AXNode* child = node->children()[i];

        bool ignore_offscreen;
        SkRect child_bounds = RelativeToTreeBoundsInternal(
            child, SkRect(), &ignore_offscreen, clip_bounds,
  ...
  while (node != nullptr) {
    ...
    SkRect container_bounds = container->data().relative_bounds.bounds;
    bounds.offset(container_bounds.x(), container_bounds.y());

    int scroll_x = 0;
    int scroll_y = 0;
    if (container->data().GetIntAttribute(ax::IntAttribute::kScrollX,
                                          &scroll_x) &&
        container->data().GetIntAttribute(ax::IntAttribute::kScrollY,
                                          &scroll_y)) {
    ...
    if (container->data().GetBoolAttribute(ax::BoolAttribute::kClipsChildren)) {
      if (!intersection.isEmpty()) {
        ...
        float x = clipped.x();
        float y = clipped.y();
        float w = clipped.width();
        float h = clipped.height();
        if (clipped.x() >= container_bounds.width()) {
          x = container_bounds.right() - 1;
          w = 1;
  ...
  if (bounds.width() == 0 && bounds.height() == 0) {
    ...
    if (ancestor && allow_recursion) {
      ...
      ancestor_bounds = RelativeToTreeBoundsInternal(
          ancestor, SkRect(), &ignore_offscreen, clip_bounds, allow_recursion);

      SkRect original_bounds = original_node->data().relative_bounds.bounds;
      if (original_bounds.x() == 0 && original_bounds.y() == 0) {
        bounds = ancestor_bounds;
      } else {
        bounds.setXYWH(bounds.x(), bounds.y(),
                       std::max(0.0f, ancestor_bounds.right() - bounds.x()),
...
SkRect AXTree::RelativeToTreeBounds(const AXNode* node,
                                    SkRect bounds,
                                    bool* offscreen,
...
SkRect AXTree::GetTreeBounds(const AXNode* node,
                             bool* offscreen,
                             bool clip_bounds) const {
  return RelativeToTreeBounds(node, SkRect(), offscreen, clip_bounds);
}

std::set<int32_t> AXTree::GetReverseRelations(ax::IntAttribute attr,
...
std::set<int32_t> AXTree::GetReverseRelations(ax::IntListAttribute attr,
                                              int32_t dst_id) const {
...
bool AXTree::Unserialize(const AXTreeUpdate& update) {
  ...
  FML_DCHECK(!GetFromId(update.root_id) == update_state.root_will_be_created);
...
AXTableInfo* AXTree::GetTableInfo(const AXNode* const_table_node) const {
  FML_DCHECK(!GetTreeUpdateInProgressState());
...
AXNode* AXTree::CreateNode(AXNode* parent,
  ...
  FML_DCHECK(!GetFromId(id));
  FML_DCHECK(update_state->GetPendingCreateNodeCount(id) > 0);
  FML_DCHECK(update_state->InvalidatesUnignoredCachedValues(id));
  FML_DCHECK(!parent ||
...
bool AXTree::UpdateNode(const AXNodeData& src,
  ...
  if (src.GetBoolAttribute(ax::BoolAttribute::kIsPageBreakingObject))
...
void AXTree::NotifyNodeDataHasBeenChanged(AXNode* node,
  ...
  if (old_data.state != new_data.state) {
    for (int32_t i = static_cast<int32_t>(ax::State::kNone) + 1;
         i <= static_cast<int32_t>(ax::State::kMaxValue); ++i) {
      ax::State state = static_cast<ax::State>(i);
  ...
  auto string_callback = [this, node](ax::StringAttribute attr,
  ...
  auto bool_callback = [this, node](ax::BoolAttribute attr,
                                    const bool& old_bool,
                                    const bool& new_bool) {
    for (AXTreeObserver* observer : observers_)
  ...
  auto float_callback = [this, node](ax::FloatAttribute attr,
  ...
  auto int_callback = [this, node](ax::IntAttribute attr, const int& old_int,
                                   const int& new_int) {
    for (AXTreeObserver* observer : observers_)
  ...
  auto stringlist_callback =
      [this, node](ax::StringListAttribute attr,
...
void AXTree::UpdateReverseRelations(AXNode* node, const AXNodeData& new_data) {
  ...
  int id = new_data.id;
  auto int_callback = [this, id](ax::IntAttribute attr, const int& old_id,
                                 const int& new_id) {
  ...
  auto intlist_callback = [this, id](ax::IntListAttribute attr,
  ...
  auto string_callback = [this, id](ax::StringAttribute attr,
                                    const std::string& old_string,
                                    const std::string& new_string) {
    if (attr == ax::StringAttribute::kChildTreeId) {
...
void AXTree::DestroySubtree(AXNode* node, AXTreeUpdateState* update_state) {
  ...
  // changes and invalidations to apply. If any of these are missing, observers
  // may not be notified of changes.
  FML_DCHECK(update_state);
  FML_DCHECK(update_state->GetPendingDestroySubtreeCount(node->id()) > 0);
  FML_DCHECK(!node->parent() || update_state->InvalidatesUnignoredCachedValues(
                                    node->parent()->id()));
...
void AXTree::SetEnableExtraMacNodes(bool enabled) {
  ...
  FML_DCHECK(0U == table_info_map_.size());
...
void AXTree::RecursivelyPopulateOrderedSetItemsMap(
  ...
  for (AXNode::UnignoredChildIterator itr =
    ...
    // are never higher.
    if (child->data().HasState(ax::State::kInvisible) &&
    ...
    if (child->data().role == ax::Role::kComment ||
        (original_node.data().role == ax::Role::kRadioButton &&
         child->data().role == ax::Role::kRadioButton) ||
        (original_node.data().role != ax::Role::kRadioButton &&
...
void AXTree::ComputeSetSizePosInSetAndCache(const AXNode& node,
  ...
  if (node.data().role != ax::Role::kComment &&
      node.data().role != ax::Role::kRadioButton &&
  ...
  if (node.data().role == ax::Role::kPopUpButton &&
    ...
    if (set_content && set_content->set_items_.size() == 1) {
      ...
      if (menu_list_popup->data().role == ax::Role::kMenuListPopup) {
...
void AXTree::ComputeSetSizePosInSetAndCacheHelper(
  ...
  for (const AXNode* item : ordered_set_content.set_items_) {
    ...
    int32_t pos_in_set_value = std::max(
        num_elements + 1, item->GetIntAttribute(ax::IntAttribute::kPosInSet));
    ...
    if (item->GetHierarchicalLevel() &&
        item->HasIntAttribute(ax::IntAttribute::kPosInSet))
      pos_in_set_value = item->GetIntAttribute(ax::IntAttribute::kPosInSet);
    ...
    max_item_set_size_from_attribute =
        std::max(max_item_set_size_from_attribute,
                 item->GetIntAttribute(ax::IntAttribute::kSetSize));
  ...
  if (const AXNode* ordered_set = ordered_set_content.ordered_set_) {
    set_size_value =
        std::max(set_size_value,
                 ordered_set->GetIntAttribute(ax::IntAttribute::kSetSize));
  ...
  for (const AXNode* item : ordered_set_content.set_items_) {
    ...
    if (item->GetHierarchicalLevel() &&
        item->HasIntAttribute(ax::IntAttribute::kSetSize))
...
std::optional<int> AXTree::GetPosInSet(const AXNode& node) {
  if (node.data().role == ax::Role::kPopUpButton &&
      node.GetUnignoredChildCount() == 0 &&
      node.HasIntAttribute(ax::IntAttribute::kPosInSet)) {
    return node.GetIntAttribute(ax::IntAttribute::kPosInSet);
  }
...
std::optional<int> AXTree::GetSetSize(const AXNode& node) {
  if (node.data().role == ax::Role::kPopUpButton &&
      node.GetUnignoredChildCount() == 0 &&
      node.HasIntAttribute(ax::IntAttribute::kSetSize)) {
    return node.GetIntAttribute(ax::IntAttribute::kSetSize);
  }
  ...
  if (node.data().role == ax::Role::kPopUpButton) {
    const auto& controls_ids =
        node.data().GetIntListAttribute(ax::IntListAttribute::kControlsIds);
...
AXTree::Selection AXTree::GetUnignoredSelection() const {
  ...
  if (anchor_position->IsIgnored()) {
    ...
    if (anchor_position->IsTextPosition() &&
        anchor_position->GetAnchor()->data().role == ax::Role::kInlineTextBox) {
    ...
    switch (anchor_position->kind()) {
      case AXPositionKind::NULL_POSITION:
        ...
        unignored_selection.anchor_affinity = ax::TextAffinity::kDownstream;
        unignored_selection.focus_object_id = AXNode::kInvalidAXID;
        unignored_selection.focus_offset = -1;
        unignored_selection.focus_affinity = ax::TextAffinity::kDownstream;
  ...
  if (focus_position->IsIgnored()) {
    ...
    if (focus_position->IsTextPosition() &&
        focus_position->GetAnchor()->data().role == ax::Role::kInlineTextBox) {
