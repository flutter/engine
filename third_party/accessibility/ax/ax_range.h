...
#ifndef ACCESSIBILITY_AX_AX_RANGE_H_
#define ACCESSIBILITY_AX_AX_RANGE_H_
...
class AXRangeRectDelegate {
 public:
  virtual SkRect GetInnerTextRangeBoundsRect(
      AXTreeID tree_id,
      AXNode::AXID node_id,
      int start_offset,
      int end_offset,
      AXOffscreenResult* offscreen_result) = 0;
  virtual SkRect GetBoundsRect(AXTreeID tree_id,
                               AXNode::AXID node_id,
                               AXOffscreenResult* offscreen_result) = 0;
};
 ...
 public:
  ...
  std::u16string GetText(AXTextConcatenationBehavior concatenation_behavior =
    ...
    while (!start->IsNullPosition()) {
      FML_DCHECK(start->IsLeafTextPosition());
      FML_DCHECK(start->text_offset() >= 0);
  ...
  std::vector<SkRect> GetRects(AXRangeRectDelegate* delegate) const {
    std::vector<SkRect> rects;

    for (const AXRange& leaf_text_range : *this) {
      FML_DCHECK(leaf_text_range.IsLeafTextRange());
...
#endif  // ACCESSIBILITY_AX_AX_RANGE_H_
