...
size_t FindAccessibleTextBoundary(const base::string16& text,
  ...
  if (boundary == ax::TextBoundary::kCharacter ||
      boundary == ax::TextBoundary::kSentenceStart ||
      boundary == ax::TextBoundary::kWordStart ||
      boundary == ax::TextBoundary::kWordStartOrEnd) {
    if (!break_iter.Init())
      return start_offset;
  }

  if (boundary == ax::TextBoundary::kLineStart) {
    if (direction == ax::MoveDirection::kForward) {
      for (int line_break : line_breaks) {
        size_t clamped_line_break = size_t{std::max(0, line_break)};
        if ((affinity == ax::TextAffinity::kDownstream &&
             clamped_line_break > start_offset) ||
            (affinity == ax::TextAffinity::kUpstream &&
             clamped_line_break >= start_offset)) {
    ...
    switch (boundary) {
      ...
      case ax::TextBoundary::kCharacter:
        if (break_iter.IsGraphemeBoundary(result)) {
          ...
          if (direction == ax::MoveDirection::kBackward ||
              result != start_offset)
            return result;
        }
        break;
