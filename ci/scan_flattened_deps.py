...
def sarif_rule():
  ...
  return {
    ...
    'help': {
      'text': 'More details in the OSV DB at: https://osv.dev/vulnerability/'
    },
...
def parse_deps_file(deps_flat_file):
  ...
  responses = requests.post(osv_url, headers=headers,
    json={'queries': queries}, allow_redirects=True)
  if responses.status_code != 200:
    ...
    if len(filtered_results)>0:
      print('Found {vuln_count} vulnerabilit(y/ies), adding to report'.format(
  ...
  with open(DEPS,'r', encoding='utf-8') as file:
    ...
    if UPSTREAM_PREFIX + dep_name in deps :
      try:
        ...
        dep_roll_date = subprocess.check_output('git show -s --format=%ct {dep}'
          .format(dep=dep[1]), shell=True).decode()
        print('dep roll date is {dep_roll_date}'.format(dep_roll_date=dep_roll_date))
        years = (time.time() - int(dep_roll_date)) / 31556952 # convert to years since last roll
        if years >= 1:
          print('Old dep found: {depUrl} is from {dep_roll_date}'
            .format(depUrl=dep[0], dep_roll_date=dep_roll_date))
        ...
        default_branch = subprocess.check_output('git remote show upstream ' +
          "| sed -n \'/HEAD branch/s/.*: //p\'", shell=True).decode()
        ...
        commit = subprocess.check_output('git for-each-ref ' +
          "--format=\'%(objectname:short)\' refs/heads/upstream", shell=True)
        ...
        ancestor_commit = subprocess.check_output('git merge-base {commit} {depUrl}'
          .format(commit=commit, depUrl=dep[1]), shell=True)
