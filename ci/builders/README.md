...
## Triaging global generators locally

Global generators are scripts that can run locally once they have all the sub-build dependencies are downloaded. This section explains how to download dependencies to a local gclient checkout.

### Prerequisites (one time installation)

CAS client is required to download the sub-build artifacts.
...
Create an ensure.txt file with the following content:
```
infra/tools/luci/cas/mac-amd64 rP7SUASC3hxbjEyHDYyjOW3dr8vNgBmb5FpdxY3eFwgC
```

Run the following command to download the dependency from the cipd servers:
```
cipd ensure -root . -ensure-file ensure.txt
# Add $HOME/tools to path
```

#### Gclient engine checkout

### Download sub-builds to gclient checkout out folder

CAS sub-build artifacts can be downloaded using information from a LUCI build e.g. in https://ci.chromium.org/p/flutter/builders/prod/Mac%20mac_ios_engine/2818 the execution details from steps 13 - 17 show the commands to download the archives. `-dir` needs to point to the relative or full path to the out folder in your gclient checkout. For our example we will assume you have already cd|pushd to you local out folder:   
...
You can make local changes, add debug information and inspect the locally generated artifacts.