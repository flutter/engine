...
abstract class FormatChecker {
  ...
  Future<List<String>> getFileList(List<String> types) async {
    ...
    return output
        .split('\n')
        .where((String line) => line.isNotEmpty && !line.contains('third_party'))
        .toList();
  }
...
class GnFormatChecker extends FormatChecker {
  GnFormatChecker({
    ...
    gnBinary = File(
      path.join(repoDir.absolute.path, 'third_party', 'gn', Platform.isWindows ? 'gn.exe' : 'gn'),
    );
  }
...
class PythonFormatChecker extends FormatChecker {
  PythonFormatChecker({
    ...
    yapfBin = File(
      path.join(repoDir.absolute.path, 'tools', Platform.isWindows ? 'yapf.bat' : 'yapf.sh'),
    );
    _yapfStyle = File(path.join(repoDir.absolute.path, '.style.yapf'));
  }
