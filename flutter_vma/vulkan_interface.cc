...
#define ACQUIRE_PROC_SUFFIX(name, suffix, instance, device)    \
  fFunctions.f##name = reinterpret_cast<PFN_vk##name##suffix>( \
      getProc("vk" #name #suffix, instance, device))

VulkanInterface::VulkanInterface(skgpu::VulkanGetProc getProc,
                                 VkInstance instance,
                                 VkDevice device,
  ...
  ACQUIRE_PROC(EnumerateInstanceExtensionProperties, VK_NULL_HANDLE,
               VK_NULL_HANDLE);
  ACQUIRE_PROC(EnumerateInstanceLayerProperties, VK_NULL_HANDLE,
               VK_NULL_HANDLE);
  ...
  ACQUIRE_PROC(GetPhysicalDeviceImageFormatProperties, instance,
               VK_NULL_HANDLE);
  ACQUIRE_PROC(GetPhysicalDeviceProperties, instance, VK_NULL_HANDLE);
  ACQUIRE_PROC(GetPhysicalDeviceQueueFamilyProperties, instance,
               VK_NULL_HANDLE);
  ACQUIRE_PROC(GetPhysicalDeviceMemoryProperties, instance, VK_NULL_HANDLE);
  ACQUIRE_PROC(GetPhysicalDeviceSparseImageFormatProperties, instance,
               VK_NULL_HANDLE);
  ...
  if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0)) {
    ...
    ACQUIRE_PROC(GetPhysicalDeviceImageFormatProperties2, instance,
                 VK_NULL_HANDLE);
    ACQUIRE_PROC(GetPhysicalDeviceQueueFamilyProperties2, instance,
                 VK_NULL_HANDLE);
    ACQUIRE_PROC(GetPhysicalDeviceMemoryProperties2, instance, VK_NULL_HANDLE);
    ACQUIRE_PROC(GetPhysicalDeviceSparseImageFormatProperties2, instance,
                 VK_NULL_HANDLE);
  } else if (extensions->hasExtension(
                 VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME, 1)) {
    ACQUIRE_PROC_SUFFIX(GetPhysicalDeviceFeatures2, KHR, instance,
                        VK_NULL_HANDLE);
    ACQUIRE_PROC_SUFFIX(GetPhysicalDeviceProperties2, KHR, instance,
                        VK_NULL_HANDLE);
    ACQUIRE_PROC_SUFFIX(GetPhysicalDeviceFormatProperties2, KHR, instance,
                        VK_NULL_HANDLE);
    ACQUIRE_PROC_SUFFIX(GetPhysicalDeviceImageFormatProperties2, KHR, instance,
                        VK_NULL_HANDLE);
    ACQUIRE_PROC_SUFFIX(GetPhysicalDeviceQueueFamilyProperties2, KHR, instance,
                        VK_NULL_HANDLE);
    ACQUIRE_PROC_SUFFIX(GetPhysicalDeviceMemoryProperties2, KHR, instance,
                        VK_NULL_HANDLE);
    ACQUIRE_PROC_SUFFIX(GetPhysicalDeviceSparseImageFormatProperties2, KHR,
  ...
  if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0)) {
    ...
    ACQUIRE_PROC_SUFFIX(GetImageMemoryRequirements2, KHR, VK_NULL_HANDLE,
                        device);
    ACQUIRE_PROC_SUFFIX(GetBufferMemoryRequirements2, KHR, VK_NULL_HANDLE,
                        device);
    ACQUIRE_PROC_SUFFIX(GetImageSparseMemoryRequirements2, KHR, VK_NULL_HANDLE,
                        device);
  }
  ...
  if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0)) {
    ...
    ACQUIRE_PROC_SUFFIX(GetDescriptorSetLayoutSupport, KHR, VK_NULL_HANDLE,
                        device);
  }

  // Functions for VK_KHR_external_memory_capabilities
  if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0)) {
    ACQUIRE_PROC(GetPhysicalDeviceExternalBufferProperties, instance,
                 VK_NULL_HANDLE);
  } else if (extensions->hasExtension(
                 VK_KHR_EXTERNAL_MEMORY_CAPABILITIES_EXTENSION_NAME, 1)) {
    ACQUIRE_PROC_SUFFIX(GetPhysicalDeviceExternalBufferProperties, KHR,
                        instance, VK_NULL_HANDLE);
  }
  ...
  if (physicalDeviceVersion >= VK_MAKE_VERSION(1, 1, 0)) {
    ...
    ACQUIRE_PROC_SUFFIX(CreateSamplerYcbcrConversion, KHR, VK_NULL_HANDLE,
                        device);
    ACQUIRE_PROC_SUFFIX(DestroySamplerYcbcrConversion, KHR, VK_NULL_HANDLE,
                        device);
  }
  ...
  if (extensions->hasExtension(
          ...
          2)) {
    ACQUIRE_PROC_SUFFIX(GetAndroidHardwareBufferProperties, ANDROID,
                        VK_NULL_HANDLE, device);
    ACQUIRE_PROC_SUFFIX(GetMemoryAndroidHardwareBuffer, ANDROID, VK_NULL_HANDLE,
                        device);
  }
#endif
}
...
#define RETURN_FALSE_INTERFACE                                           \
  if (kIsDebug) {                                                        \
    ("%s:%d VulkanInterface::validate() failed.\n", __FILE__, __LINE__); \
  }                                                                      \
  return false;
...
bool VulkanInterface::validate(uint32_t instanceVersion,
  ...
  if (nullptr == fFunctions.fCreateInstance ||
      ...
      nullptr == fFunctions.fFreeMemory || nullptr == fFunctions.fMapMemory ||
      ...
      nullptr == fFunctions.fSetEvent || nullptr == fFunctions.fResetEvent ||
      ...
      nullptr == fFunctions.fCmdDraw || nullptr == fFunctions.fCmdDrawIndexed ||
