...
/**
 * Skia Vulkan uses the following interface to make all calls into Vulkan. When
 * a Ganesh or Graphite Context is created it is given a VulkanInterface. All
 * functions that should be available based on the Vulkan's version must be
 * non-NULL or Context creation will fail. This can be tested with the
 * validate() method.
 */
struct VulkanInterface : public SkRefCnt {
 private:
  ...
  class VkPtr {
   public:
    ...
    // NOLINTNEXTLINE
    operator FNPTR_TYPE() const { return ptr_; }
 ...
 public:
  VulkanInterface(skgpu::VulkanGetProc getProc,
                  VkInstance instance,
                  VkDevice device,
  ...
  // Validates that the VulkanInterface supports its advertised standard. This
  // means the necessary function pointers have been initialized for Vulkan
  // version.
  ...
  struct Functions {
    ...
    VkPtr<PFN_vkGetImageSparseMemoryRequirements>
    ...
    VkPtr<PFN_vkGetPhysicalDeviceFormatProperties2>
    ...
    VkPtr<PFN_vkGetPhysicalDeviceMemoryProperties2>
    ...
    VkPtr<PFN_vkGetImageSparseMemoryRequirements2>
    ...
    VkPtr<PFN_vkGetMemoryAndroidHardwareBufferANDROID>
