...
#include "impeller/display_list/canvas.h"
    >>>>>>> 1df749a00e3216d18b77dc9d3de3fbf60174f3b4
...
#define UNIMPLEMENTED \
  ...
  static FilterContents::BlurStyle ToBlurStyle(
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::saveLayer(
      const DlRect& bounds, const flutter::SaveLayerOptions& options,
      uint32_t total_content_depth, flutter::DlBlendMode max_content_mode,
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::transform2DAffine(DlScalar mxx, DlScalar mxy,
                                           DlScalar mxt, DlScalar myx,
                                           DlScalar myy, DlScalar myt) {
    AUTO_DEPTH_WATCHER(0u);
  ...
  void DlDispatcherBase::transformFullPerspective(
      DlScalar mxx, DlScalar mxy, DlScalar mxz, DlScalar mxt, DlScalar myx,
      DlScalar myy, DlScalar myz, DlScalar myt, DlScalar mzx, DlScalar mzy,
      DlScalar mzz, DlScalar mzt, DlScalar mwx, DlScalar mwy, DlScalar mwz,
      DlScalar mwt) {
    AUTO_DEPTH_WATCHER(0u);
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::clipRect(const DlRect& rect, ClipOp clip_op,
                                  bool is_aa) {
    AUTO_DEPTH_WATCHER(0u);
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::clipOval(const DlRect& bounds, ClipOp clip_op,
                                  bool is_aa) {
    AUTO_DEPTH_WATCHER(0u);
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::clipRoundRect(const DlRoundRect& rrect, ClipOp sk_op,
                                       bool is_aa) {
    AUTO_DEPTH_WATCHER(0u);
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::clipPath(const DlPath& path, ClipOp sk_op,
                                  bool is_aa) {
    AUTO_DEPTH_WATCHER(0u);
  ...
  void DlDispatcherBase::drawDashedLine(const DlPoint& p0, const DlPoint& p1,
                                        DlScalar on_length,
                                        DlScalar off_length) {
    AUTO_DEPTH_WATCHER(1u);
  ...
  void DlDispatcherBase::SimplifyOrDrawPath(Canvas & canvas, const DlPath& path,
                                            const Paint& paint) {
    DlRect rect;
  ...
  void DlDispatcherBase::drawArc(const DlRect& oval_bounds,
                                 DlScalar start_degrees, DlScalar sweep_degrees,
                                 bool use_center) {
    AUTO_DEPTH_WATCHER(1u);
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::drawPoints(PointMode mode, uint32_t count,
    ...
    switch (mode) {
      case flutter::DlCanvas::PointMode::kPoints: {
        ...
        auto point_style = paint.stroke_cap == Cap::kRound
                               ? PointStyle::kRound
                               : PointStyle::kSquare;
        ...
        GetCanvas().DrawPoints(skia_conversions::ToPoints(points, count),
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::drawImage(
      const sk_sp<flutter::DlImage> image, const DlPoint& point,
      flutter::DlImageSampling sampling, bool render_with_attributes) {
    AUTO_DEPTH_WATCHER(1u);
    ...
    const auto dest =
        DlRect::MakeXYWH(point.x, point.y, size.width, size.height);
  ...
  void DlDispatcherBase::drawImageRect(
      const sk_sp<flutter::DlImage> image, const DlRect& src, const DlRect& dst,
      flutter::DlImageSampling sampling, bool render_with_attributes,
  ...
  void DlDispatcherBase::drawImageNine(const sk_sp<flutter::DlImage> image,
                                       const DlIRect& center, const DlRect& dst,
    ...
    converter.DrawNinePatch(
        ...
        Rect::MakeLTRB(center.GetLeft(), center.GetTop(), center.GetRight(),
                       center.GetBottom()),
        dst, ToSamplerDescriptor(filter), &GetCanvas(), &paint_);
  }

  // |flutter::DlOpReceiver|
  void DlDispatcherBase::drawAtlas(
      const sk_sp<flutter::DlImage> atlas, const SkRSXform xform[],
      const DlRect tex[], const flutter::DlColor colors[], int count,
      flutter::DlBlendMode mode, flutter::DlImageSampling sampling,
      const DlRect* cull_rect, bool render_with_attributes) {
    AUTO_DEPTH_WATCHER(1u);
  ...
  void DlDispatcherBase::drawDisplayList(
      const sk_sp<flutter::DisplayList> display_list, DlScalar opacity) {
    ...
    if (display_list->has_rtree() && !initial_matrix_.HasPerspective()) {
      ...
      if (global_culling_bounds.has_value()) {
        ...
        display_list->Dispatch(
            *this,
            SkRect::MakeLTRB(cull_rect.GetLeft(), cull_rect.GetTop(),
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::drawTextBlob(const sk_sp<SkTextBlob> blob, DlScalar x,
                                      DlScalar y) {
  ...
  void DlDispatcherBase::drawTextFrame(
      const std::shared_ptr<TextFrame>& text_frame, DlScalar x, DlScalar y) {
    AUTO_DEPTH_WATCHER(1u);
  ...
  // |flutter::DlOpReceiver|
  void DlDispatcherBase::drawShadow(
      const DlPath& path, const flutter::DlColor color,
      const DlScalar elevation, bool transparent_occluder, DlScalar dpr) {
    AUTO_DEPTH_WATCHER(1u);
    ...
    // Compute the spot color -- ported from SkShadowUtils::ComputeTonalColors.
    {
      ...
      Scalar alpha_adjust = (2.6f + (-2.66667f + 1.06667f * spot_color.alpha) *
                                        spot_color.alpha) *
                            spot_color.alpha;
      Scalar color_alpha =
  ...
  CanvasDlDispatcher::CanvasDlDispatcher(
      ContentContext & renderer, RenderTarget & render_target,
      bool has_root_backdrop_filter, flutter::DlBlendMode max_root_blend_mode,
      IRect cull_rect)
  ...
  void TextFrameDispatcher::drawTextFrame(
      const std::shared_ptr<impeller::TextFrame>& text_frame, DlScalar x,
      DlScalar y) {
  ...
  // |flutter::DlOpReceiver|
  void TextFrameDispatcher::setImageFilter(
  ...
  std::shared_ptr<Texture> DisplayListToTexture(
      const sk_sp<flutter::DisplayList>& display_list, ISize size,
      AiksContext& context, bool reset_host_buffer, bool generate_mips) {
    int mip_count = 1;
    if (generate_mips) {
  ...
  bool RenderToOnscreen(ContentContext & context, RenderTarget render_target,
                        ...
                        SkIRect cull_rect, bool reset_host_buffer) {
