...
void main() {
  ...
  for (float16_t i = 0.0hf; i <= blur_info.blur_radius; i += 2.0hf) {
    ...
    vec2 neg_c1 = v_texture_coords - offset_1;
    vec2 neg_c2 = v_texture_coords - offset_2;

    vec2 pos_coord = (w1 * pos_c1 + w2 * pos_c2) / gaussian;
    vec2 neg_coord = (w1 * neg_c1 + w2 * neg_c2) / gaussian;

    gaussian_integral += (gaussian + gaussian);
    total_color += gaussian * Sample(texture_sampler,  // sampler
                                     pos_coord         // texture coordinates
                              );
    total_color += gaussian * Sample(texture_sampler,  // sampler
