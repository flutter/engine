...
out f16vec2 v_dst_texture_coords;
out f16vec2 v_src_texture_coords;

void main() {
  gl_Position = frame_info.mvp * vec4(vertices, 0.0, 1.0);
  v_dst_texture_coords =
      f16vec2(IPRemapCoords(dst_texture_coords, frame_info.dst_y_coord_scale));
  v_src_texture_coords =
      f16vec2(IPRemapCoords(src_texture_coords, frame_info.src_y_coord_scale));
}
