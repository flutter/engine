...
std::string CompilerSkSL::compile() {
  ...
  statement("half4 main(float2 ignored)");
  begin_scope();
  statement("  __main();");
...
void CompilerSkSL::emit_header() {
  ...
  // //third_party/skia/src/sksl/sksl_rt_shader.sksl. gl_FragCoord is translated
  // to sk_FragCoord instead of using the argument to 'main' passed in the
  // SkSL universe.
  statement("layout(builtin=15) float4 sk_FragCoord;");
  statement("");
}
