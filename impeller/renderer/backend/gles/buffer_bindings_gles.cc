...
bool BufferBindingsGLES::ReadUniformsBindings3(const ProcTableGLES& gl,
                                               GLuint program) {
  ...
  return ReadUniformsBindings(gl, program, /*sampler_only=*/true);
}
...
bool BufferBindingsGLES::ReadUniformsBindings(const ProcTableGLES& gl,
  ...
  for (GLint i = 0; i < uniform_count; i++) {
    ...
    if (sampler_only && uniform_type != GL_SAMPLER_2D) {
      continue;
    }
...
bool BufferBindingsGLES::BindUniformDataDirect(
  ...
  for (auto i = 0u; i < buffer_count; i++) {
    if (!BindUniformBuffer3(gl, buffers[i].view)) {
      return false;
    }
  }
...
bool BufferBindingsGLES::BindUniformBuffer3(const ProcTableGLES& gl,
  ...
  if (ubo_locations_.find(metadata->name) == ubo_locations_.end()) {
    for (const auto& [key, value] : ubo_locations_) {
      FML_LOG(ERROR) << key;
    }
  }
  const auto& [block_index, binding_point] = ubo_locations_[metadata->name];
  gl.UniformBlockBinding(program_, block_index, binding_point);
  ...
  gl.BindBufferRange(
      GL_UNIFORM_BUFFER, binding_point, device_buffer_gles.GetID(),
      buffer.resource.GetRange().offset, buffer.resource.GetRange().length);
  return true;
}
