...
DebugReportVK::Result DebugReportVK::OnDebugCallback(
  ...
  // TODO(csg): This is a real issue caused by INPUT_ATTACHMENT_BIT not being a
  // supported `VkSurfaceCapabilitiesKHR::supportedUsageFlags` on any platform
  // other than Android. This is necessary for all the framebuffer fetch related
  // tests. We can get away with suppressing this on macOS but this must be
  // fixed.
  ...
  // Issue in MoltenVK where the ID number is zero and the message warns of
  // emulation.
  if (data->messageIdNumber == 0 &&                                //
      data->pMessage != nullptr &&                                 //
      strstr(data->pMessage, "MTLCounterSampleBuffer") != nullptr  //
  ) {
    return Result::kContinue;
  }
