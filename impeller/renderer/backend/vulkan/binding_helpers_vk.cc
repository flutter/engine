...
static bool BindImages(BindingOffsets offsets,
  ...
  for (auto i = 0u; i < offsets.length; i++) {
    const BoundTexture& bound_texture = bound_textures[i + offsets.offset];
    const auto& texture_vk = TextureVK::Cast(*bound_texture.texture.resource);
    const SamplerVK& sampler = SamplerVK::Cast(*bound_texture.sampler);

    if (!encoder->Track(bound_texture.texture.resource) ||
        !encoder->Track(sampler.GetSharedSampler())) {
      return false;
    }

    const SampledImageSlot& slot = bound_texture.slot;
...
static bool BindBuffers(BindingOffsets offsets,
                        ...
                        std::vector<vk::WriteDescriptorSet>& writes) {
  for (auto i = 0u; i < offsets.length; i++) {
    const BoundBuffer& bound_buffer = bound_buffers[i + offsets.offset];
    ...
    // ShaderUniformSlot.
    const ShaderUniformSlot& uniform = bound_buffer.slot;
    auto layout_it =
...
fml::StatusOr<std::vector<vk::DescriptorSet>> AllocateAndBindDescriptorSets(
  ...
  size_t textures_count = bound_textures.size();
  ...
  auto descriptor_result = encoder->AllocateDescriptorSets(
      buffer_count, textures_count, subpass_count, layouts);
  ...
  std::vector<vk::DescriptorBufferInfo> buffers;
  std::vector<vk::WriteDescriptorSet> writes;
  images.reserve(textures_count + subpass_count);
  buffers.reserve(buffer_count);
  writes.reserve(textures_count + buffer_count + subpass_count);
  ...
  for (const auto& command : commands) {
    ...
    if (!BindBuffers(command.buffer_bindings, bound_buffers, allocator, encoder,
                     descriptor_sets[desc_index], desc_set, buffers, writes)) {
      return fml::Status(fml::StatusCode::kUnknown, "Failed to bind buffer.");
    }
    if (!BindImages(command.texture_bindings, bound_textures, allocator,
                    encoder, descriptor_sets[desc_index], images, writes)) {
      return fml::Status(fml::StatusCode::kUnknown, "Failed to bind texture.");
    }
