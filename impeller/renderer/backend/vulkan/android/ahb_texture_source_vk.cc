...
AHBTextureSourceVK::AHBTextureSourceVK(const std::shared_ptr<Context>& context,
  ...
  auto yuv_conversion = CreateYUVConversion(context_vk, ahb_props);
  ...
  context_vk.SetDebugName(device_memory_.get(), "AHB Device Memory");
  context_vk.SetDebugName(image_.get(), "AHB Image");
  context_vk.SetDebugName(yuv_conversion_->GetConversion(),
                          "AHB YUV Conversion");
  context_vk.SetDebugName(image_view_.get(), "AHB ImageView");
...
AHBTextureSourceVK::AHBTextureSourceVK(
    ...
    std::shared_ptr<android::HardwareBuffer> hardware_buffer)
    : AHBTextureSourceVK(context,
                         hardware_buffer->GetHandle(),
                         hardware_buffer->GetAndroidDescriptor()) {
  hardware_buffer_ = std::move(hardware_buffer);
  is_valid_ = is_valid_ && hardware_buffer_ && hardware_buffer_->IsValid();
}

// |TextureSourceVK|
AHBTextureSourceVK::~AHBTextureSourceVK() = default;

const std::shared_ptr<android::HardwareBuffer>&
AHBTextureSourceVK::GetHardwareBuffer() const {
