...
class AHBTextureSourceVK final : public TextureSourceVK {
 public:
  ...
  AHBTextureSourceVK(const std::shared_ptr<Context>& context,
                     std::shared_ptr<android::HardwareBuffer> buffer);

  // |TextureSourceVK|
  ~AHBTextureSourceVK() override;

  const std::shared_ptr<android::HardwareBuffer>& GetHardwareBuffer() const;
 ...
 private:
  std::shared_ptr<android::HardwareBuffer> hardware_buffer_;
