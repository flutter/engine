...
class VulkanBackbuffer {
 public:
  ...
  const VulkanHandle<VkFence>& UsageFence() const;

  const VulkanHandle<VkFence>& RenderFence() const;

  const VulkanHandle<VkSemaphore>& UsageSemaphore() const;

  const VulkanHandle<VkSemaphore>& RenderSemaphore() const;

  VulkanCommandBuffer& UsageCommandBuffer();
