...
void main() {
  ...
  test('Locale unnamed constructor idiosyncrasies', () {
    expect(() => Locale.parse(Locale('en', '').toString()), throwsException,
        reason: 'Locale("en", "").toString() produces "en-" which is not '
            'standards-compliant.');

    expect(Locale.parse(Locale('en', '').toLanguageTag()), Locale('en'),
    ...
    expect(Locale('abcd').toLanguageTag(), 'abcd',
        reason: 'Locale("abcd") is not following instructions in the API '
            'documentation, so produces standards-uncompliant output.');
    expect(Locale.parse('abcd').toLanguageTag(), 'und-Abcd', reason: '');
    expect(Locale('abcd'), isNot(Locale.parse('abcd')), reason: '');
    expect(() => Locale.parse(Locale('a').toLanguageTag()), throwsException,
        reason: 'Locale("abcd") is not following instructions in the API '
            'documentation, so produces standards-uncompliant output.');
    expect(Locale.parse(Locale('EN').toLanguageTag()).languageCode, 'en',
    ...
    expect(Locale.parse('da-u-nu').toLanguageTag(), equals('da-u-nu'),
  ...
  group('Locale.parse():', () {
    test('languageCode.', () {
      expect(Locale.parse('IW').languageCode, 'he',
          reason: "Case insensitive to input.");
      expect(Locale.parse('Fil').languageCode, 'fil',
          reason: "3-character language codes: Filipino is not Finnish.");
      expect(Locale.parse('abcde').languageCode, 'abcde',
          reason: 'The spec provides for language codes with 5 to 8 '
              'characters, though as of 2018, there aren\'t any valid tags '
              'this long.');
      expect(Locale.parse('ROOT').languageCode, 'und',
          reason: 'BCP 47 Language Tag Conversion: '
              'replace "root" with "und", and case insensitive.');
    });

    test('scriptCode.', () {
      expect(Locale.parse('af_latn').scriptCode, 'Latn');
      expect(Locale.parse('zh_HANT-CN').scriptCode, 'Hant');
      expect(Locale.parse('sw-TZ').scriptCode, null);
    });

    test('countryCode.', () {
      expect(Locale.parse('ar-Arab-EG').countryCode, 'EG');
      expect(Locale.parse('en-GB_scouse_fonipa').countryCode, 'GB');
    });
    ...
    test('Locale Identifiers with extensions.', () {
      ...
      expect(Locale.parse('ca-u-kb-true').toLanguageTag(), equals('ca-u-kb'),
          reason: 'For true/false keys like kb, true can be ommitted.');
      expect(Locale.parse('ca-u-kb').toLanguageTag(), equals('ca-u-kb'),
          reason: 'For true/false keys like kb, true can be ommitted.');
  ...
  test('Equality.', () {
    expect(Locale.parse('en'), isNot(Locale.parse('en-Latn')));
    expect(
        Locale.parse('en').hashCode, isNot(Locale.parse('en-Latn').hashCode));

    expect(Locale.parse('en'), isNot(Locale.parse('en-US')));
    expect(Locale.parse('en').hashCode, isNot(Locale.parse('en-US').hashCode));

    expect(Locale.parse('en'), isNot(Locale.parse('en-fonipa')));
    expect(
        Locale.parse('en').hashCode, isNot(Locale.parse('en-fonipa').hashCode));

    expect(Locale.parse('en'), isNot(Locale.parse('en-a')));
    expect(Locale.parse('en').hashCode, isNot(Locale.parse('en-a').hashCode));

    expect(Locale.parse('en'), isNot(Locale.parse('en-a')));
    expect(Locale.parse('en').hashCode, isNot(Locale.parse('en-a').hashCode));

    expect(Locale.parse('en-u-attr'), isNot(Locale.parse('en-u-nu-roman')));
    expect(Locale.parse('en-u-attr').hashCode,
        isNot(Locale.parse('en-u-nu-roman').hashCode));

    expect(Locale.parse('en-u-kb'), Locale.parse('en-u-kb-true'),
        reason: '-u-kb should parse to the same result as -u-kb-true.');
    expect(
        Locale.parse('en-u-kb').hashCode, Locale.parse('en-u-kb-true').hashCode,
        reason: '-u-kb should parse to the same result as -u-kb-true.');

    expect(Locale.parse('en-t-hi'), isNot(Locale.parse('en-t-hi-h0-hybrid')));
    expect(Locale.parse('en-t-hi').hashCode,
