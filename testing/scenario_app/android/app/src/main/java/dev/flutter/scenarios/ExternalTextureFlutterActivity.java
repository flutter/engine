...
public class ExternalTextureFlutterActivity extends TestActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // Add the text view to the column layout
        TextView text = new TextView(getContext());
        text.setText("surface=" + surfaceRenderer + ", top=flutter, bottom=android");
        text.setTextSize(20);
        text.setTextColor(Color.WHITE);
        text.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT)); // Set width to match parent
        columnLayout.addView(text);

        // Add the surface view to the column layout
        columnLayout.addView(surfaceView,
                new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                        ViewGroup.LayoutParams.WRAP_CONTENT,
                        Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL));

        // Add the column layout to the frame layout
        ...
        surfaceHolder.addCallback(
                new SurfaceRendererCallback(surfaceViewRenderer, firstFrameLatch));
    }
        ...
        switch (surfaceRenderer) {
            case "image":
                if (VERSION.SDK_INT >= VERSION_CODES.M) {
                    // CanvasSurfaceRenderer doesn't work correctly when used with
                    // ImageSurfaceRenderer. Use MediaSurfaceRenderer for now.
            ...
            case "media":
                if (VERSION.SDK_INT >= VERSION_CODES.LOLLIPOP) {
                    return new MediaSurfaceRenderer(
                            this::createMediaExtractor, extras.getInt("rotation", 0));
                } else {
    ...
    private static class CanvasSurfaceRenderer implements SurfaceRenderer {
        ...
        @Override
        public void repaint() {
            Canvas canvas = VERSION.SDK_INT >= VERSION_CODES.M ? surface.lockHardwareCanvas()
                                                               : surface.lockCanvas(null);
            Paint paint = new Paint();
            paint.setShader(new LinearGradient(0.0f, 0.0f, canvas.getWidth(), canvas.getHeight(),
                    new int[] {
                            // Cyan (#00FFFF)
                            0xFF00FFFF,
                            // Magenta (#FF00FF)
    ...
    private static class MediaSurfaceRenderer implements SurfaceRenderer {
        ...
        public void attach(Surface surface, CountDownLatch onFirstFrame) {
            ...
            // NOTE: MediaFormat.KEY_ROTATION was not available until 23+, but the key is still
            // handled on API 21+.
        ...
        private void decodeThreadMain() {
            try {
                ...
                while (true) {
                    // Move samples (video frames) from the extractor into the decoder, as long as
                    // we haven't consumed all samples.
                    if (!seenEOS) {
                    ...
                    // Then consume decoded video frames from the decoder. These frames are
                    // automatically pushed to the attached Surface, so this schedules them for
                    // present.
                    ...
                    if (outputBufferIndex >= 0) {
                        if (bufferInfo.size > 0) {
                            ...
                            codec.releaseOutputBuffer(outputBufferIndex,
                                    startTimeNs + (bufferInfo.presentationTimeUs * 1000));
                        }
                    }
    ...
    private static class ImageSurfaceRenderer implements SurfaceRenderer {
        ...
        public void attach(Surface surface, CountDownLatch onFirstFrame) {
            ...
            if (VERSION.SDK_INT >= VERSION_CODES.Q) {
                ...
                reader = ImageReader.newInstance(SURFACE_WIDTH, SURFACE_HEIGHT, ImageFormat.PRIVATE,
                        2, HardwareBuffer.USAGE_GPU_SAMPLED_IMAGE);
            } else {
                ...
                reader = ImageReader.newInstance(
                        SURFACE_WIDTH, SURFACE_HEIGHT, writer.getFormat(), 2);
            }
        ...
        private void onImageAvailable(ImageReader reader) {
            ...
            if (!canWriteImage) {
                // If the ImageWriter hasn't released the latest image, don't attempt to enqueue
                // another image. Otherwise the reader writer pair locks up if the writer runs
                // behind, as the reader runs out of images and the writer has no more space for
                // images.
                canReadImage = true;
                return;
            }
    ...
    private static class SurfaceRendererCallback implements SurfaceHolder.Callback {
        ...
        public SurfaceRendererCallback(
                SurfaceRenderer surfaceRenderer, CountDownLatch onFirstFrame) {
