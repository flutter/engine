...
- (void)testAccessibilityFocusOnTextSemanticsProducesCorrectIosViews {
  ...
  // frame.
  XCUIElement* textInputSemanticsObject = self.application.textFields[@"flutter textfield"];
  ...
  XCUIElement* delegateTextInput = self.application.textViews.firstMatch;
  XCTAssertTrue([delegateTextInput ftr_waitForNonExistenceWithTimeout:timeout]);

  // Nor should there be a keyboard for text entry.
  XCUIElement* keyboard = self.application.keyboards.firstMatch;
  ...
  delegateTextInput = self.application.textViews.firstMatch;
  ...
  keyboard = self.application.keyboards.firstMatch;
