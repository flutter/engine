  ...
  dispatch_async(self.displayQueue, ^{
    if(self.glContext == NULL){
      ...
      if(flutterShareGroup != NULL){
        self.glContext = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES3 sharegroup:flutterShareGroup];
      }
      else{
    ...
    if(self.materialTexture == 0){
      ...
      if(materialImage != NULL){
        [self convertCGImage:materialImage.CGImage toTexture:self.materialTexture inSize:materialImage.size];
      }
    }
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, self.frameBuffer);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, self.frameTexture, 0);
