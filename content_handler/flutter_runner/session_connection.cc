...
SessionConnection::SessionConnection(
    const ui_mozart::MozartPtr& mozart,
...
void SessionConnection::OnSessionEvents(
  ...
  using Type = scenic::Event::Tag;

  for (auto& event : events) {
    switch (event->get_scenic()->which()) {
      case Type::METRICS: {
        if (event->get_scenic()->get_metrics()->node_id == root_node_.id()) {
          auto& metrics = event->get_scenic()->get_metrics()->metrics;
...
void SessionConnection::Present(flow::CompositorContext::ScopedFrame& frame) {
  ...
  session_.Present(0,  // presentation_time. (placeholder).
                   [](ui_mozart::PresentationInfoPtr) {}  // callback
  );
