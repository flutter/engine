...
void RuntimeHolder::Init(
  ...
  if (!Dart_IsPrecompiledRuntime()) {
    ...
    if (!directory_asset_bundle_->GetAsBuffer(kDylibKey, &dylib_blob)) {
...
void RuntimeHolder::CreateView(
  ...
  if (!Dart_IsPrecompiledRuntime()) {
    if (!directory_asset_bundle_->GetAsBuffer(kKernelKey, &kernel) &&
        !directory_asset_bundle_->GetAsBuffer(kSnapshotKey, &snapshot)) {
