...
VulkanSurface::VulkanSurface(vulkan::VulkanProcTable& p_vk,
  ...
  if (!AllocateDeviceMemory(std::move(context), size)) {
...
bool VulkanSurface::AllocateDeviceMemory(sk_sp<GrContext> context,
...

  {
    ...
    if (VK_CALL_LOG_ERROR(vk.CreateImage(backend_context_->fDevice,
    ...
    vk_image_ = {vk_image, [this](VkImage image) {
                   vk.DestroyImage(backend_context_->fDevice, image, NULL);
                 }};
  }

  // Create the memory.
  VkMemoryRequirements memory_reqs;
  vk.GetImageMemoryRequirements(backend_context_->fDevice,  //
                                vk_image_,                  //
                                &memory_reqs                //
                                );
...

  {
    ...
    if (VK_CALL_LOG_ERROR(vk.AllocateMemory(backend_context_->fDevice,
    ...
    vk_memory_ = {vk_memory, [this](VkDeviceMemory memory) {
                    vk.FreeMemory(backend_context_->fDevice, memory, NULL);
                  }};
  }

  // Bind image memory.
  if (VK_CALL_LOG_ERROR(vk.BindImageMemory(backend_context_->fDevice, vk_image_,
                                           vk_memory_, 0)) != VK_SUCCESS) {
    return false;
  }

  {
    ...
    if (VK_CALL_LOG_ERROR(vk.ExportDeviceMemoryMAGMA(
            backend_context_->fDevice, vk_memory_, &vmo_handle)) !=
        VK_SUCCESS) {
      return false;
    }
    exported_vmo_.reset(static_cast<mx_handle_t>(vmo_handle));
  }

  // Assert that the VMO size was sufficient.
  size_t vmo_size = 0;
  if (exported_vmo_.get_size(&vmo_size) != MX_OK ||
