...
void RasterizerDirect::Draw(flow::Pipeline<flow::LayerTree>* pipeline) {
  TRACE_EVENT0("flutter", "RasterizerDirect::Draw");

  if (platform_view_ == nullptr || pipeline == nullptr) {
    return;
  }

  flow::Pipeline<flow::LayerTree>::Consumer consumer =
      std::bind(&RasterizerDirect::DoDraw, this, std::placeholders::_1);

  // Consume as many pipeline items as possible. But yield the event loop
  // between successive tries.
  switch (pipeline->Consume(consumer)) {
    case flow::PipelineConsumeResult::MoreAvailable:
      blink::Threads::Gpu()->PostTask(
          std::bind(&RasterizerDirect::Draw, this, pipeline));
      break;
    default:
      break;
  }
