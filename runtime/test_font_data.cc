...
std::vector<sk_sp<SkTypeface>> GetTestFontData() {
  ...
  // This font is currently distributed via flutter_tools, and is only available
  // to framework tests.
  const char* flutter_root_from_env = std::getenv("FLUTTER_ROOT");
  if (flutter_root_from_env) {
    const std::string flutter_root(flutter_root_from_env);
    typefaces.push_back(SkTypeface::MakeFromFile(
        (flutter_root + "/bin/cache/artifacts/test_fonts/flutter_test.ttf")
            .c_str()));
  }
#endif  // EMBED_TEST_FONT_DATA
  return typefaces;
