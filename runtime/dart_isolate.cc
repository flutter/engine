...
bool DartIsolate::PrepareForRunningFromSource(
  ...
  // The "source" file may be a ".dill" file.
  if (FileNameIsDill(main_source_absolute_path)) {
    auto mapping =
        std::make_unique<fml::FileMapping>(main_source_absolute_path, false);
    if (mapping == nullptr) {
      return false;
    }
    if (!LoadKernelSnapshot(std::move(mapping))) {
      return false;
    }
  } else {
...
DartIsolate::CreateDartVMAndEmbedderObjectPair(
  ...
  Dart_Isolate isolate =
      vm->GetPlatformKernel() != nullptr
          ...
          : Dart_CreateIsolate(
                ...
                vm->GetIsolateSnapshot().GetInstructionsIfPresent(),       //
                flags,                                                     //
                embedder_isolate,                                          //
                error                                                      //
            );
  ...
  return {isolate, embedder_isolate->GetWeakIsolatePtr()};
}
