...
// Spawns child isolates from root though dart, and check for termination
// of child isolates after destruction of the root.
TEST_F(DartIsolateTest, RootIsolateChildrenDestroyedOnShutdown) {
  Settings settings = {};
  settings.task_observer_add = [](intptr_t, fml::closure) {};
  settings.task_observer_remove = [](intptr_t) {};
  auto vm = DartVM::ForProcess(settings);
  ...
  // TODO(garyq): Create children isolates and destroy the root, check
  // if children isolates are also terminated.

  // This test currently does not test anything new!
