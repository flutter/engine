...
std::unique_ptr<flutter::LayerTree> Scene::takeLayerTree(int64_t view_id) {
  ...
  auto layer_tree_config = std::move(layer_tree_config_);

  return std::make_unique<LayerTree>(
      std::move(*layer_tree_config),
