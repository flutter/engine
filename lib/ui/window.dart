...
///
/// When constructed with "safe" constructors, instances of this Locale class
/// will produce standards-compliant output, even for garbage input. By
/// contrast, the unnamed constructor follows the "Garbage In, Garbage Out"
/// principle: getter promises are broken if unacceptable parameters are passed
/// to this constructor.
///
...
class Locale {
  ...
  ///
  /// If const constructability is not needed, use the safe version of this
  /// constructor: [Locale.safe].
  const Locale(this._languageCode, [ this._countryCode ]) : assert(_languageCode != null);

  /// WIP/FIXME: document the constructor properly as it settles down in review.
  ///
  /// This constructor matches the unnamed constructor, but does not follow the
  /// "garbage in, garbage out". This class guarantees standards-compliant
  /// output if instances are constructed with safe constructors. (The various
  /// outputs of the Locale instance can be trusted to be syntactically valid
  /// even if the input is garbage.)
  ///
  /// In its current state, this code throws away bad tags. Bad language tags
  /// become "und" (undefined). Bad region tags are discarded giving a
  /// regionless Locale.
  ///
  /// Other options we could consider is throwing exceptions: it would be nice
  /// to provide feedback to calling code shoud it want feedback, so that it
  /// could make its own decision on how to deal with garbage.
  Locale.safe(String language, { String region }) :
    _languageCode = _canonicalizeLanguageSubtag(language),
    _countryCode = _canonicalizeRegionSubtag(region);
  ...
  static String _canonicalizeLanguageSubtag(String languageCode) {
    if (languageCode.length < 2 ||
        languageCode.length == 4 ||
        languageCode.length > 8) {
      // We have no need to preserve the "root" subtag, and we promise BCP 47
      // compliant output. Consequently we might as well do the replacement
      // immediately.
      return 'und';
    }
    return _replaceDeprecatedLanguageSubtag(languageCode.toLowerCase());
  }

  static String _replaceDeprecatedLanguageSubtag(String languageCode) {
    switch (languageCode) {
      // The cases below are generated by //flutter/tools/gen_locale.dart
