...
export class FlutterEntrypointLoader {
  ...
  async load(build, options) {
    let { onEntrypointLoaded, nonce } = options;
    onEntrypointLoaded ??= (engineInitializer) => {
      engineInitializer.initializeEngine(options).then((appRunner) => appRunner.runApp())
    };
    console.log(`TARGET: ${build.compileTarget}`);
    if (build.compileTarget == "dart2wasm") {
      return this._loadWasmEntrypoint(build, onEntrypointLoaded);
    } else {
      const mainPath = build.mainJsPath ?? "main.dart.js";
      const entrypointUrl = `${baseUri}${mainPath}`;
      console.log(`ENTRYPOINT URL: ${entrypointUrl}`);
      return this._loadJSEntrypoint(entrypointUrl, onEntrypointLoaded, nonce);
    }
  }
  ...
  async _loadWasmEntrypoint(build, onEntrypointLoaded) {
    console.log("load wasm entrypoint");
    if (!this._scriptLoaded) {
      console.log("script not yet loaded");
      this._scriptLoaded = true;
      ...
      console.log("about to load support runtime");
      const jsSupportRuntime = await import(jsSupportRuntimeUri);
      console.log("about to instantiate module");
      const moduleInstance = await jsSupportRuntime.instantiate(dartModulePromise, {});
