...
export class FlutterLoader {
  ...
  async load({
    ...
    const buildIsCompatible = (build) => {
      ...
      if (options.renderer && options.renderer != build.renderer) {
        return false;
      }
    ...
    deps.serviceWorkerLoader.setTrustedTypesPolicy(deps.flutterTT);
    await deps.serviceWorkerLoader.loadServiceWorker(serviceWorkerSettings).catch(e => {
      // Regardless of what happens with the injection of the SW, the show must go on
      console.warn("Exception while loading service worker:", e);
    });
    if (build.renderer == "canvaskit") {
      deps.canvasKit = loadCanvasKit(deps, options, browserEnvironment);
    } else if (build.renderer == "skwasm") {
      deps.skwasm = loadSkwasm();
    }
    ...
    this.didCreateEngineInitializer =
      entrypointLoader.didCreateEngineInitializer.bind(entrypointLoader);
    return entrypointLoader.loadEntrypoint({
      onEntrypointLoaded,
      entrypointUrl: build.url ?? "/main.dart.js",
    });
  }
}
