...
export class FlutterLoader {
  ...
  async load({
    ...
    if (build.renderer == "canvaskit") {
      deps.canvasKit = loadCanvasKit(deps, config, browserEnvironment);
    } else if (build.renderer == "skwasm") {
      deps.skwasm = loadSkwasm(deps, config, browserEnvironment);
    }
    ...
    this.didCreateEngineInitializer =
      entrypointLoader.didCreateEngineInitializer.bind(entrypointLoader);
    return entrypointLoader.load(build, deps, config, onEntrypointLoaded);
  }
}
