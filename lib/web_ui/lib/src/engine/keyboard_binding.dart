...
class _KeyboardConverter {
  ...
  // List<html.KeyboardEvent> _syncModifierKeys(html.KeyboardEvent event) {
  // }
  ...
  bool handleEvent(html.KeyboardEvent event) {
    ...
    final bool isDownEvent = event.type == 'keydown';
    assert(isDownEvent || event.type == 'keyup');

    late final ui.KeyData keyData;

    if (isDownEvent) {
      if (_pressedKeys.containsKey(physicalKey)) {
        if (event.repeat ?? false) {
          final int pressedLogicalKey = _pressedKeys[physicalKey]!;
          if (pressedLogicalKey == logicalKey) {
            ...
            logicalKeyDataList..add(ui.LogicalKeyData(
              ...
              key: pressedLogicalKey,
            ))..add(ui.LogicalKeyData(
              change: ui.KeyChange.synchronize,
              key: logicalKey,
              character: character,
            ));
          ...
          assert(_pressedKeys[physicalKey] == logicalKey);
        } else {
          // Down event on a pressed key, and not repeated: skip
        }
      } else {
        logicalKeyDataList.add(ui.LogicalKeyData(
      ...
      if (_pressedKeys.containsKey(physicalKey)) {
        ...
        // Up event on a released key: skip
      }
    }
