...
const String kPhysicalCapsLock = 'CapsLock';
...
class KeyboardConverter {
  ...
  VoidCallback scheduleAsyncEvent(Duration duration, ui.KeyData data, VoidCallback callback) {
    bool canceled = false;
    Future<void>.delayed(duration).then<void>((_) {
      if (!canceled && !_disposed) {
        callback();
        dispatchKeyData(data);
      }
    });
  ...
  bool handleEvent(FlutterHtmlKeyboardEvent event) {
    ...
    if (event.code! == kPhysicalCapsLock && _shouldSynthesizeCapsLockCancel()) {
      assert(isPhysicalDown);
      ...
      final ui.KeyData postSynthesizedKeyData = ui.KeyData(
        timeStamp: timeStamp,
        change: ui.KeyChange.cancel,
        key: physicalKey,
        lockFlags: 0,
