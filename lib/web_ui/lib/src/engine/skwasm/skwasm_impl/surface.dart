...
@pragma('wasm:export')
WasmVoid callbackHandler(
    WasmI32 callbackId, WasmI32 context, WasmExternRef? jsContext) {
  ...
  if (FlutterConfiguration.flutterWebUseSkwasm) {
    SkwasmCallbackHandler.instance
        .handleCallback(callbackId, context, jsContext);
  }
  return WasmVoid();
}
...
class SkwasmCallbackHandler {
  ...
  factory SkwasmCallbackHandler._() {
    final WasmFuncRef wasmFunction = WasmFunction<
        WasmVoid Function(
            WasmI32, WasmI32, WasmExternRef?)>.fromFunction(callbackHandler);
  ...
  final Map<CallbackId, Completer<JSAny>> _pendingCallbacks =
      <int, Completer<JSAny>>{};
  ...
  void handleCallback(
    ...
    final Completer<JSAny> completer =
        _pendingCallbacks.remove(callbackId.toIntUnsigned())!;
    if (!jsContext.isNull) {
...
class SkwasmSurface {
  ...
  SkwasmSurface._fromHandle(this.handle)
      : threadId = surfaceGetThreadId(handle);
  final SurfaceHandle handle;

  final int threadId;

  void _initialize() {
    surfaceSetCallbackHandler(
        handle, SkwasmCallbackHandler.instance.callbackPointer);
  }
  ...
  Future<RenderResult> renderPictures(List<SkwasmPicture> pictures) =>
      withStackScope((StackScope scope) async {
        ...
        final int callbackId =
            surfaceRenderPictures(handle, pictureHandles, pictures.length);
        final RasterResult rasterResult = (await SkwasmCallbackHandler.instance
            .registerCallback(callbackId)) as RasterResult;
        final RenderResult result = (
          imageBitmaps: rasterResult.imageBitmaps.toDart.cast<DomImageBitmap>(),
          rasterStartMicros:
              (rasterResult.rasterStartMilliseconds.toDartDouble * 1000)
                  .toInt(),
          rasterEndMicros:
              (rasterResult.rasterEndMilliseconds.toDartDouble * 1000).toInt(),
        );
        return result;
      });

  Future<ByteData> rasterizeImage(
      SkwasmImage image, ui.ImageByteFormat format) async {
    final int callbackId = surfaceRasterizeImage(
      handle,
      image.handle,
      format.index,
    );
    final int context = (await SkwasmCallbackHandler.instance
            .registerCallback(callbackId) as JSNumber)
        .toDartInt;
    ...
    final Pointer<Uint8> dataPointer =
        skDataGetConstPointer(dataHandle).cast<Uint8>();
