...
WasmVoid callbackHandler(
  ...
  if (FlutterConfiguration.flutterWebUseSkwasm) {
    SkwasmCallbackHandler.instance.handleCallback(
      callbackId,
      context,
      jsContext,
    );
...
class SkwasmCallbackHandler {
  ...
  factory SkwasmCallbackHandler._() {
    final WasmFuncRef wasmFunction = WasmFunction<
      WasmVoid Function(WasmI32, WasmI32, WasmExternRef?)
    >.fromFunction(callbackHandler);
...
class SkwasmSurface {
  ...
  void _initialize() {
    surfaceSetCallbackHandler(
      handle,
      SkwasmCallbackHandler.instance.callbackPointer,
    );
  }

  Future<RenderResult> renderPictures(
    List<SkwasmPicture> pictures,
    ...
    final RenderResult result = (
      ...
      rasterStartMicros:
          (rasterResult.rasterStartMilliseconds.toDartDouble * 1000).toInt(),
      rasterEndMicros:
