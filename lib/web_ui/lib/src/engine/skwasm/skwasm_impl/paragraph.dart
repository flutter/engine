...
List<String> _computeEffectiveFontFamilies(List<String> fontFamilies) {
  ...
  final Iterable<String> filteredFonts = fontFamilies.where(
    _testFonts.contains,
  );
...
class SkwasmParagraph extends SkwasmObjectWrapper<RawParagraph>
  ...
  void layout(ui.ParagraphConstraints constraints) {
    ...
    if (!debugDisableFontFallbacks && !_hasCheckedForMissingCodePoints) {
      ...
      final int missingCodePointCount = paragraphGetUnresolvedCodePoints(
        handle,
        nullptr,
        0,
      );
      if (missingCodePointCount > 0) {
        withStackScope((StackScope scope) {
          final Pointer<Uint32> codePointBuffer = scope.allocUint32Array(
  ...
  List<ui.TextBox> _convertTextBoxList(TextBoxListHandle listHandle) {
    ...
    return withStackScope((StackScope scope) {
      ...
      return List<ui.TextBox>.generate(length, (int index) {
        final int textDirectionIndex = textBoxListGetBoxAtIndex(
          listHandle,
          index,
          tempRect,
        );
  ...
  @override
  ui.TextPosition getPositionForOffset(ui.Offset offset) => withStackScope((
    StackScope scope,
  ) {
  ...
  @override
  ui.TextRange getWordBoundary(ui.TextPosition position) => withStackScope((
    StackScope scope,
  ) {
  ...
  List<ui.TextBox> getBoxesForPlaceholders() {
    final TextBoxListHandle listHandle = paragraphGetBoxesForPlaceholders(
      handle,
    );
    final List<ui.TextBox> boxes = _convertTextBoxList(listHandle);
    textBoxListDispose(listHandle);
  ...
  ui.LineMetrics? getLineMetricsAt(int index) {
    final LineMetricsHandle lineMetrics = paragraphGetLineMetricsAtIndex(
      handle,
      index,
    );
...
class SkwasmNativeTextStyle extends SkwasmObjectWrapper<RawTextStyle> {
  ...
  factory SkwasmNativeTextStyle.defaultTextStyle() => SkwasmNativeTextStyle(
    textStyleCreate(),
  );
...
class SkwasmTextStyle implements ui.TextStyle {
  ...
  void applyToNative(SkwasmNativeTextStyle style) {
    ...
    if (locale != null) {
      final SkStringHandle localeHandle = skStringFromDartString(
    ...
    if (fontFeatures != null) {
      for (final ui.FontFeature feature in fontFeatures!) {
        final SkStringHandle featureName = skStringFromDartString(
          feature.feature,
        );
    ...
    if (fontVariations != null && fontVariations!.isNotEmpty) {
      ...
      withStackScope((StackScope scope) {
        final Pointer<Uint32> axisBuffer = scope.allocUint32Array(
          variationCount,
        );
        final Pointer<Float> valueBuffer = scope.allocFloatArray(
          variationCount,
        );
...
class SkwasmParagraphBuilder extends SkwasmObjectWrapper<RawParagraphBuilder>
  ...
  factory SkwasmParagraphBuilder(
    ...
    paragraphBuilderCreate(style.handle, collection.handle),
    style,
  );
  ...
  static final DomSegmenter _wordSegmenter = createIntlSegmenter(
    granularity: 'word',
  );
  ...
  void _addSegmenterData() => withStackScope((StackScope scope) {
    ...
    final Pointer<Uint8> utf8Data = paragraphBuilderGetUtf8Text(
      handle,
      outSize,
    );
  ...
  UnicodePositionBufferHandle _createBreakPositionBuffer(
    ...
    final Pointer<Uint32> buffer = unicodePositionBufferGetDataPointer(
      positionBuffer,
    );
  ...
  void _addLineBreakData(String text, JSString jsText) {
    ...
    final LineBreakBufferHandle lineBreakBuffer = lineBreakBufferCreate(
      lineBreaks.length + 1,
    );
    final Pointer<LineBreak> lineBreakPointer = lineBreakBufferGetDataPointer(
      lineBreakBuffer,
    );
