...
extension WebGLContextExtension on WebGLContext {
  ...
  @JS('TEXTURE0')
  external JSNumber get _texture0;
  int get texture0 => _texture0.toDart.toInt();
  
  @JS('TEXTURE_2D')
  external JSNumber get _texture2d;
  int get texture2d => _texture2d.toDart.toInt();
  
  @JS('FRAMEBUFFER')
  external JSNumber get _framebuffer;
  int get framebuffer => _framebuffer.toDart.toInt();
  
  @JS('COLOR_ATTACHMENT0')
  external JSNumber get _colorAttachment0;
  int get colorAttachment0 => _colorAttachment0.toDart.toInt();
  
  @JS('RGBA')
  external JSNumber get _rgba;
  int get rgba => _rgba.toDart.toInt();
  
  @JS('UNSIGNED_BYTE')
  external JSNumber get _unsignedByte;
  int get unsignedByte => _unsignedByte.toDart.toInt();

  @JS('activeTexture')
  external void _activeTexture(JSNumber texture);
  void activeTexture(int texture) => _activeTexture(texture.toJS);
  
  @JS('createTexture')
  external WebGLTexture? _createTexture();
  WebGLTexture? createTexture() => _createTexture();
  
  @JS('bindTexture')
  external void _bindTexture(JSNumber target, WebGLTexture? texture);
  void bindTexture(int target, WebGLTexture? texture) => _bindTexture(target.toJS, texture);
  
  @JS('createFramebuffer')
  external WebGLFramebuffer? _createFramebuffer();
  WebGLFramebuffer? createFramebuffer() => _createFramebuffer();
  
  @JS('framebufferTexture2D')
  external void _framebufferTexture2D(JSNumber target, JSNumber attachment, JSNumber textarget,
      WebGLTexture? texture, JSNumber level);
  void framebufferTexture2D(int target, int attachment, int textarget,
      WebGLTexture? texture, int level) => _framebufferTexture2D(target.toJS, attachment.toJS, textarget.toJS, texture, level.toJS);
  
  @JS('texImage2D')
  external void _texImage2D(JSNumber target, JSNumber level, JSNumber internalformat, JSNumber format,
      JSNumber type, DomTexImageSource source);
  void texImage2D(int target, int level, int internalformat, int format,
      int type, DomTexImageSource source) => _texImage2D(target.toJS, level.toJS, internalformat.toJS, format.toJS, type.toJS, source);
  
  @JS('bindFramebuffer')
  external void _bindFramebuffer(JSNumber target, WebGLFramebuffer? framebuffer);
  void bindFramebuffer(int target, WebGLFramebuffer? framebuffer) => _bindFramebuffer(target.toJS, framebuffer);
  
  @JS('drawBuffers')
  external void _drawBuffers(List<JSNumber> buffers);
  void drawBuffers(List<int> buffers) => _drawBuffers(buffers.map((int i) => i.toJS).toList());
  
  @JS('readPixels')
  external void _readPixels(JSNumber x, JSNumber y, JSNumber width, JSNumber height, JSNumber format,
      JSNumber type, TypedData? pixels);
  void readPixels(int x, int y, double width, double height, int format,
      int type, TypedData? pixels) => _readPixels(x.toJS, y.toJS, width.toJS, height.toJS, format.toJS, type.toJS, pixels);
