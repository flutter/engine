...
/// Convenience function for making a fetch request and getting the data as a
/// [DomBlob], when the default error handling mechanism is sufficient.
Future<DomBlob> httpFetchBlob(String url) async {
  final HttpFetchResponse response = await httpFetch(url);
...
extension HttpFetchResponseExtension on HttpFetchResponse {
  ...
  /// Returns the data parsed as blob.
  Future<DomBlob> blob() {
    return payload.blob();
  }
...
class HttpFetchPayloadImpl implements HttpFetchPayload {
  ...
  /// Return the data as a blob.
  @override
  Future<DomBlob> blob() => _domResponse.blob();
}
...
class MockHttpFetchPayload implements HttpFetchPayload {
  ...
  @override
  Future<DomBlob> blob() async => _blob!;
}
...
extension _DomResponseExtension on _DomResponse {
  ...
  Future<DomBlob> blob() =>
      js_util.promiseToFuture(js_util.callMethod(this, 'blob', <Object>[]));
}
