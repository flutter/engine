...
abstract class BrowserImageDecoder implements ui.Codec {
  ...
  Future<ImageDecoder> _getOrCreateWebDecoder() async {
    if (_cachedWebDecoder != null) {
      ...
      // currently animating.
      _cacheExpirationClock.datetime = DateTime.now().add(
    ...
    try {
      ...
      repetitionCount =
          rawRepetitionCount == double.infinity
              ? -1
  ...
  Future<ui.FrameInfo> getNextFrame() async {
    ...
    final DecodeResult result =
        await promiseToFuture<DecodeResult>(
    ...
    final Duration duration = Duration(
      microseconds: frame.duration?.toInt() ?? 0,
    );
