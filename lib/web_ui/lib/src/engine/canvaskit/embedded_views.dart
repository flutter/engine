...
class HtmlViewEmbedder {
  ...
  Iterable<CkCanvas> getOverlayCanvases() {
    return _context.pictureRecordersCreatedDuringPreroll.map(
      (CkPictureRecorder r) => r.recordingCanvas!,
    );
  }
  ...
  void _compositeWithParams(int platformViewId, EmbeddedViewParams params) {
    ...
    final ViewClipChain clipChain = _viewClipChains.putIfAbsent(
      platformViewId,
      () {
    ...
    for (final Mutator mutator in mutators) {
      switch (mutator.type) {
        case MutatorType.transform:
          ...
          head.style.transform = float64ListToCssTransform(
            headTransform.storage,
          );
  ...
  Future<void> submitFrame(CkPicture basePicture) async {
    ...
    if (debugOverlayOptimizationBounds) {
      ...
      final CkPaint pictureBoundsPaint =
          CkPaint()..color = const ui.Color.fromARGB(100, 0, 0, 255);
        ...
        if (entity is RenderingPlatformView) {
          if (entity.debugComputedBounds != null) {
            boundsCanvas.drawRect(
              entity.debugComputedBounds!,
              platformViewBoundsPaint,
            );
          }
  ...
  Rendering _modifyRenderingForMaxCanvases(Rendering rendering) {
    ...
    final List<RenderingEntity> modifiedEntities = List<RenderingEntity>.from(
      rendering.entities,
    );
  ...
  void _updateDomForNewRendering(Rendering rendering) {
    ...
    final List<int> indexMap = _getIndexMapFromPreviousRendering(
      _activeRendering,
      rendering,
    );
    ...
    final List<int> staticElements = longestIncreasingSubsequence(
      existingIndexMap,
    );
    ...
    void updateRenderCanvasWithDisplay(
      RenderingRenderCanvas renderCanvas,
      int index,
    ) {
      ...
      if (indexMap[index] != -1) {
        ...
        assert(
          previousEntity is RenderingRenderCanvas &&
    ...
    while (staticElementIndex < staticElements.length) {
      ...
      while (indexMap[nextRenderingIndex] !=
          staticElementIndexInActiveRendering) {
  ...
  List<int> _getIndexMapFromPreviousRendering(
    ...
    final int maxUnchangedLength = math.min(
    ...
    while (index < next.entities.length) {
      ...
      for (
        ...
        if (previous.entities[oldIndex].equalsForRendering(
              next.entities[index],
            ) &&
  ...
  void debugCleanupSvgClipPaths() {
    ...
    if (parent != null) {
      for (
        DomNode? child = parent.lastChild;
        child != null;
