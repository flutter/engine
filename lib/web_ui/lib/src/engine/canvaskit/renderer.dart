...
class CanvasKitRenderer implements Renderer {
  ...
  bool _currentlyRendering = false;
  late Future<void> _currentRender;
  (ui.Scene, ui.FlutterView)? _nextRenderTask;
  ...
  Future<void> renderScene(ui.Scene scene, ui.FlutterView view) async {
    _nextRenderTask = (scene, view);
    return _kickRenderLoop();
  }

  Future<void> _kickRenderLoop() async {
    if (!_currentlyRendering) {
      if (_nextRenderTask == null) {
        return Future<void>.value();
      }
      _currentlyRendering = true;
      final ui.Scene scene = _nextRenderTask!.$1;
      final ui.FlutterView view = _nextRenderTask!.$2;
      _nextRenderTask = null;
      return _currentRender = _renderScene(scene, view).then((_) {
        _currentlyRendering = false;
        return _kickRenderLoop();
      }, onError: (Object error, StackTrace stackTrace) {
        _currentlyRendering = false;
        if (EnginePlatformDispatcher.instance.onError != null) {
          EnginePlatformDispatcher.instance.onError!(error, stackTrace);
        }
        return _kickRenderLoop();
      });
    } else {
      // The next render task is queued, but the current scene isn't done
      // rendering yet. So return the [Future] we created when we started
