...
class CanvasKitRenderer implements Renderer {
  ...
  set resourceCacheMaxBytes(int bytes) => _rasterizer.setResourceCacheMaxBytes(
    bytes,
  );
  ...
  Future<void> initialize() async {
    _initialized ??= () async {
      ...
      _onViewCreatedListener ??= viewManager.onViewCreated.listen(
        _onViewCreated,
      );
      _onViewDisposedListener ??= viewManager.onViewDisposed.listen(
        _onViewDisposed,
      );
      _instance = this;
    }();
  ...
  ui.ImageFilter composeImageFilters({
    ...
    return CkImageFilter.compose(
      outer: outer as CkImageFilter,
      inner: inner as CkImageFilter,
    );
  }
  ...
  }) async => skiaInstantiateImageCodec(list, targetWidth, targetHeight);
  ...
  FutureOr<ui.Image> createImageFromTextureSource(
    ...
    if (!transferOwnership) {
      final DomImageBitmap bitmap =
          await createImageBitmap(object, (
            x: 0,
            y: 0,
            width: width,
            height: height,
