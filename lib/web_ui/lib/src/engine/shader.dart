...
class GradientLinear extends EngineGradient {
  ...
  html.CanvasGradient createPaintStyle(html.CanvasRenderingContext2D ctx) {
    ...
    if (colorStops == null) {
      ...
      gradient.addColorStop(0, ui.colorToCssString(colors[0]));
      gradient.addColorStop(1, ui.colorToCssString(colors[1]));
      return gradient;
    }
    for (int i = 0; i < colors.length; i++) {
      gradient.addColorStop(colorStops[i], ui.colorToCssString(colors[i]));
    }
    return gradient;
  }
  ...
  List<dynamic> webOnlySerializeToCssPaint() {
    ...
    for (int i = 0; i < colors.length; i++) {
      serializedColors.add(ui.colorToCssString(colors[i]));
    }
    return <dynamic>[
      1,
      from.dx,
