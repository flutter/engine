...
class _CanvasPool extends _SaveStackTracking {
  ...
  void drawColor(ui.Color color, ui.BlendMode blendMode) {
    ...
    contextHandle.fillStyle = ui.colorToCssString(color);
    ...
    if (shadows.isNotEmpty) {
      for (final CanvasShadow shadow in shadows) {
        ...
        if (transparentOccluder && browserEngine != BrowserEngine.webkit) {
          ...
          context.strokeStyle = '';
          context.fillStyle = ui.colorToCssString(shadow.color);
          _runPath(context, path);
          context.fill();
          context.restore();
        } else {
          ...
          context.shadowColor = ui.colorToCssString(shadow.color.withAlpha(0xff));
