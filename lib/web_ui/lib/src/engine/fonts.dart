...
Future<FontManifest> fetchFontManifest(ui_web.AssetManager assetManager) async {
  ...
  if (!response.hasPayload) {
    printWarning(
      'Font manifest does not exist at `${response.url}` - ignoring.',
    );
    return FontManifest(<FontFamily>[]);
  }

  final Converter<List<int>, Object?> decoder = const Utf8Decoder().fuse(
    const JsonDecoder(),
  );
  ...
  final List<FontFamily> families =
      (fontManifestJson! as List<dynamic>).map((dynamic fontFamilyJson) {
        final Map<String, dynamic> fontFamily =
            fontFamilyJson as Map<String, dynamic>;
...
class FontDownloadError extends FontLoadError {
  ...
  @override
  String get message =>
      'Failed to download font asset at url $url with error: $error.';
}
