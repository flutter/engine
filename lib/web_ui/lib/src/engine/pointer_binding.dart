...
abstract class _BaseAdapter {
  ...
  final Map<String, DomEventListener> _listeners =
    <String, DomEventListener>{};
  /// Listeners that are registered through native javascript api.
  final Map<String, DomEventListener> _nativeListeners =
    <String, DomEventListener>{};
  final DomElement glassPaneElement;
  ...
  void clearListeners() {
    ...
    // api.
    _nativeListeners.forEach((String eventName, DomEventListener listener) {
  ...
  ///
  /// The event listener is attached to [domWindow] but only events that have
  ...
  void addEventListener(
    ...
    dynamic loggedHandler(DomEvent event) {
      if (!acceptOutsideGlasspane && !glassPaneElement.contains(event.target as DomNode?)) {
        return null;
      }
...
class _PointerAdapter extends _BaseAdapter with _WheelEventListenerMixin {
  ...
  void _addPointerEventListener(
    ...
    addEventListener(eventName, (DomEvent event) {
      final DomPointerEvent pointerEvent = event as DomPointerEvent;
  ...
  @override
  void setup() {
    _addPointerEventListener('pointerdown', (DomPointerEvent event) {
    ...
    _addPointerEventListener('pointermove', (DomPointerEvent event) {
      final int device = _getPointerId(event);
      final _ButtonSanitizer sanitizer = _ensureSanitizer(device);
      final List<ui.PointerData> pointerData = <ui.PointerData>[];
    ...
    _addPointerEventListener('pointerup', (DomPointerEvent event) {
      final int device = _getPointerId(event);
      final List<ui.PointerData> pointerData = <ui.PointerData>[];
    ...
    _addPointerEventListener('pointercancel', (DomPointerEvent event) {
      final int device = _getPointerId(event);
      final List<ui.PointerData> pointerData = <ui.PointerData>[];
...
class _TouchAdapter extends _BaseAdapter {
  ...
  @override
  void setup() {
    _addTouchEventListener('touchstart', (DomTouchEvent event) {
      final Duration timeStamp = _BaseAdapter._eventTimeStampToDuration(event.timeStamp!);
      final List<ui.PointerData> pointerData = <ui.PointerData>[];
    ...
    _addTouchEventListener('touchmove', (DomTouchEvent event) {
    ...
    _addTouchEventListener('touchend', (DomTouchEvent event) {
    ...
    _addTouchEventListener('touchcancel', (DomTouchEvent event) {
      final Duration timeStamp = _BaseAdapter._eventTimeStampToDuration(event.timeStamp!);
      final List<ui.PointerData> pointerData = <ui.PointerData>[];
      for (final DomTouch touch in event.changedTouches!.cast<DomTouch>()) {
        final bool nowPressed = _isTouchPressed(touch.identifier!);
...
class _MouseAdapter extends _BaseAdapter with _WheelEventListenerMixin {
  ...
  @override
  void setup() {
    _addMouseEventListener('mousedown', (DomMouseEvent event) {
      final List<ui.PointerData> pointerData = <ui.PointerData>[];
      final _SanitizedDetails? up =
    ...
    _addMouseEventListener('mousemove', (DomMouseEvent event) {
      final List<ui.PointerData> pointerData = <ui.PointerData>[];
    ...
    _addMouseEventListener('mouseup', (DomMouseEvent event) {
      final List<ui.PointerData> pointerData = <ui.PointerData>[];
