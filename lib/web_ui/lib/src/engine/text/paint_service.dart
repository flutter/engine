...
class TextPaintService {
  ...
  void _paintBackground(
    ...
    final background = fragment.style.background as SurfacePaint?;
    if (background != null) {
      final rect = fragment.toPaintingTextBox().toRect();
      if (!rect.isEmpty) {
  ...
  void _paintText(
    ...
    final fragmentX = fragment.textDirection! == ui.TextDirection.ltr
        ? fragment.left
        : fragment.right;

    final x = offset.dx + line.left + fragmentX;
    final y = offset.dy + line.baseline;

    final style = fragment.style;

    final text = fragment.getText(paragraph);
