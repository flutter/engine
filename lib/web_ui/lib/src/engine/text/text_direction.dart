...
final UnicodePropertyLookup<ui.TextDirection?>
_textDirectionLookup = UnicodePropertyLookup<ui.TextDirection?>(<
...
List<BidiFragment> _computeBidiFragments(String text) {
  ...
  for (int i = 1; i < text.length; i++) {
    ...
    if (charTextDirection != textDirection) {
      ...
      fragments.add(
        BidiFragment(fragmentStart, i, textDirection, fragmentFlow),
      );
      fragmentStart = i;
...
ui.TextDirection? _getTextDirection(String text, int i) {
  ...
  final ui.TextDirection? textDirection = _textDirectionLookup.findForChar(
    codePoint,
  );
