...
class BitmapCanvas extends EngineCanvas {
  ...
  DomHTMLElement _drawImage(
    ...
    final ui.BlendMode? blendMode = paint.blendMode;
    final EngineHtmlColorFilter? colorFilter = createHtmlColorFilter(
      paint.colorFilter,
    );
  ...
  void drawImageRect(
    ...
    if (dst.width == image.width &&
      ...
      final DomElement imgElement = _drawImage(
        image,
        ui.Offset(targetLeft, targetTop),
        paint,
      );
  ...
  DomHTMLElement _createImageElementWithSvgBlendFilter(
    ...
    final SvgFilter svgFilter = svgFilterFromBlendMode(
      filterColor,
  ...
  ui.Rect _computeScreenBounds(Matrix4 targetTransform) {
    ...
    final Vector3 topRight = inverted.perspectiveTransform(
      x: width,
      y: 0,
      z: 0,
    );
    final Vector3 bottomRight = inverted.perspectiveTransform(
      x: width,
      y: height,
      z: 0,
    );
    final Vector3 bottomLeft = inverted.perspectiveTransform(
      x: 0,
      y: height,
      z: 0,
    );
...
List<DomElement> _clipContent(
  ...
  for (int clipIndex = 0; clipIndex < len; clipIndex++) {
    ...
    if (rect != null) {
      ...
      newClipTransform =
          newClipTransform.clone()..translate(clipOffsetX, clipOffsetY);
      curElement.style
        ..overflow = 'hidden'
      ...
      if (surfacePath.pathRef.isOval != -1) {
        ...
        curElement.style
          ...
          ..transformOrigin = '0 0 0';
        final DomElement clipElement = createSvgClipDef(
          curElement,
          entry.path!,
        );
