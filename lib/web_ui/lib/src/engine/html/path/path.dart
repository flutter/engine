...
class SurfacePath implements ui.Path {
  ...
  void arcTo(
    ...
    for (int i = 0; i < conicCount; i++) {
      ...
      conics.add(
        Conic(p0.dx, p0.dy, p1.dx, p1.dy, p2.dx, p2.dy, quadrantWeight),
      );
    }
    ...
    if (dot < 1) {
      ...
      final double unscaledLength = math.sqrt(
        (offCurveX * offCurveX) + (offCurveY * offCurveY),
      );
  ...
  @override
  SurfacePath shift(ui.Offset offset) => SurfacePath.shiftedFrom(
    this,
    offset.dx,
    offset.dy,
  );
  ...
  int _internalGetConvexity() {
    ...
    if (pointCount > 3) {
      ...
      --pointIndex;
      final int convexity = Convexicator.bySign(
        pathRef,
        pointIndex,
