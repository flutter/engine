...
class GradientSweep extends EngineGradient {
  ...
  Object createImageBitmap(
    ...
    final GlContext gl = _sharedCanvas.checkOutContext(
      widthInPixels,
      heightInPixels,
    );
    gl.setViewportSize(widthInPixels, heightInPixels);

    final NormalizedGradient normalizedGradient = NormalizedGradient(
      colors,
      stops: colorStops,
    );
    ...
    final Object tileOffset = gl.getUniformLocation(
      glProgram.program,
      'u_tile_offset',
    );
    ...
    final Object gradientMatrix = gl.getUniformLocation(
      glProgram.program,
      'm_gradient',
    );
    ...
    if (matrix4 != null) {
      final Matrix4 m4 =
          Matrix4.zero()..copyInverse(Matrix4.fromFloat32List(matrix4!));
  ...
  String _createSweepFragmentShader(
    ...
    final String probeName = _writeSharedGradientShader(
      builder,
      method,
      gradient,
      tileMode,
...
class GradientLinear extends EngineGradient {
  ...
  Object createImageBitmap(
    ...
    final Matrix4 originTranslation = Matrix4.translationValues(
      -originX,
      -originY,
      0,
    );
    ...
    if (matrix4 != null) {
      ...
      final Matrix4 m4 =
          Matrix4.zero()..copyInverse(Matrix4.fromFloat32List(matrix4!.matrix));
    ...
    final Object uRes = gl.getUniformLocation(
      glProgram.program,
      'u_resolution',
    );
  ...
  if (colorStops == null) {
    ...
    for (int i = 0; i < colors.length; i++) {
      ...
      gradient.addColorStop(
        colorStop * scale + offset,
        colors[i].toCssString(),
      );
    }
  }
  if (isDecal) {
...
class GradientRadial extends EngineGradient {
  ...
  Object createImageBitmap(
    ...
    final Object radiusUniform = gl.getUniformLocation(
      glProgram.program,
      'u_radius',
    );
...
class GradientConical extends GradientRadial {
  ...
  String _createRadialFragmentShader(
    ...
    if (centerDistanceSq < kFltEpsilonSquared) {
      return super._createRadialFragmentShader(
        gradient,
        shaderBounds,
        tileMode,
      );
