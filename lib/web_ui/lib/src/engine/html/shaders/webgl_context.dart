...
class GlContext {
  ...
  Object compileShader(String shaderType, String source) {
    ...
    final bool shaderStatus = js_util
        .callMethod(glContext, 'getShaderParameter', <dynamic>[shader, compileStatus]) as bool;
    if (!shaderStatus) {
  ...
  void linkProgram(Object program) {
    ...
    final bool programDidLink = js_util
        .callMethod(glContext, 'getProgramParameter', <dynamic>[program, kLinkStatus]) as bool;
    if (!programDidLink) {
  ...
  /// Destroys gl context.
  void dispose() {
    js_util.callMethod(_getExtension('WEBGL_lose_context') as Object, 'loseContext', const <dynamic>[]);
  }
