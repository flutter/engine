...
Future<void> testMain() async {
  ...
  test(
    'Computes paint bounds for draw line when line exceeds limits',
    () async {
      // Uses max bounds when computing paint bounds
      final RecordingCanvas rc = RecordingCanvas(screenRect);
      rc.drawLine(
        const Offset(50, 100),
        const Offset(screenWidth + 100.0, 140),
        testPaint,
      );
      rc.endRecording();
      // The off by one is due to the minimum stroke width of 1.
  ...
  test('drawColor should cover full size', () async {
    final RecordingCanvas rc = RecordingCanvas(screenRect);
    final SurfacePaint testPaint =
        SurfacePaint()..color = const Color(0xFF80FF00);
    rc.drawRect(const Rect.fromLTRB(10, 20, 30, 40), testPaint);
    ...
    () async {
      ...
      expect(
        ...
        Rect.fromLTRB(
          textLeft,
          textTop,
          textLeft + paragraph.maxIntrinsicWidth,
          21.0,
        ),
      );
  ...
  test(
    'Should draw a Rect for straight line when strokeWidth is zero.',
    () async {
      final RecordingCanvas rc = RecordingCanvas(screenRect);
  ...
  test(
    'Should support reusing path and reset when drawing into canvas.',
    () async {
      final RecordingCanvas rc = RecordingCanvas(
        const Rect.fromLTRB(0, 0, 100, 100),
      );
...
typedef PaintSpreadPainter =
    void Function(RecordingCanvas canvas, SurfacePaint paint);
