...
Future<void> testMain() async {
  ...
  test('Paints sweep gradient rectangles', () async {
    ...
    GradientSweep sweepGradient = GradientSweep(const Offset(0.5, 0.5), colors,
        stops, TileMode.clamp, 0, 360.0 / 180.0 * math.pi, null);
    ...
    canvas.drawRect(
        ...
        SurfacePaint()
          ..shader = engineGradientToShader(
              ...
              Rect.fromLTWH(rectBounds.center.dx, rectBounds.top,
                  rectBounds.width / 2, rectBounds.height)));
    canvas.drawRect(rectBounds, borderPaint);

    // Gradient with start/endangle.
    sweepGradient = GradientSweep(const Offset(0.5, 0.5), colors, stops,
        TileMode.clamp, math.pi / 6, 3 * math.pi / 4, null);
    ...
    sweepGradient = GradientSweep(const Offset(0.5, 0.5), colors, stops,
        TileMode.repeated, math.pi / 6, 3 * math.pi / 4, null);
    ...
    rectBounds = rectBounds.translate(kBoxWidth + 10, 0);
    sweepGradient = GradientSweep(const Offset(0.5, 0.5), colors, stops,
        TileMode.mirror, math.pi / 6, 3 * math.pi / 4, null);
    canvas.drawRect(
        rectBounds,
        SurfacePaint()
          ..shader = engineGradientToShader(sweepGradient, rectBounds));
    ...
    canvas.restore();
    await canvasScreenshot(canvas, 'sweep_gradient_rect',
        canvasRect: screenRect, region: region);
  }, skip: isFirefox);
  ...
  test('Paints sweep gradient ovals', () async {
    ...
    canvas.restore();
    await canvasScreenshot(canvas, 'sweep_gradient_oval',
        canvasRect: screenRect, region: region);
  }, skip: isFirefox);
  ...
  test('Paints sweep gradient paths', () async {
    ...
    canvas.restore();
    await canvasScreenshot(canvas, 'sweep_gradient_path',
        canvasRect: screenRect, region: region);
  }, skip: isFirefox);
  ...
  test('Paints rotated and shifted linear gradient', () async {
    ...
    canvas.restore();
    await canvasScreenshot(canvas, 'linear_gradient_rect_shifted',
        canvasRect: screenRect, region: region);
  }, skip: isFirefox);
  ...
  test('Paints linear gradient properly when within svg context', () async {
    ...
    canvas.restore();
    await canvasScreenshot(canvas, 'linear_gradient_in_svg_context',
        canvasRect: screenRect, region: region);
  }, skip: isFirefox);
...
Shader engineLinearGradientToShader(GradientLinear gradient, Rect rect) {
  return Gradient.linear(
    ...
    gradient.colorStops,
    gradient.tileMode,
    gradient.matrix4 == null
        ? null
        : Float64List.fromList(gradient.matrix4!.matrix),
  );
}
...
Path samplePathFromRect(Rect rectBounds) => Path()
