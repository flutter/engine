...
Future<void> testMain() async {
  ...
  test(
    'pushClipRect with offset and transform ClipOp none should not clip',
    () async {
      final SurfaceSceneBuilder builder = SurfaceSceneBuilder();
      ...
      await matchGoldenFile(
        'compositing_clip_rect_clipop_none.png',
        region: region,
      );
    },
  );
  ...
  test(
    'pushClipRRect with offset and transform ClipOp none should not clip',
    () async {
      final SurfaceSceneBuilder builder = SurfaceSceneBuilder();
      ...
      await matchGoldenFile(
        'compositing_clip_rrect_clipop_none.png',
        region: region,
      );
    },
  );
  ...
  test('pushImageFilter using mode ColorFilter', () async {
    ...
    builder.pushImageFilter(
      const ui.ColorFilter.mode(ui.Color(0xFFFF0000), ui.BlendMode.srcIn),
    );
    _drawTestPicture(builder);
    builder.pop();
...
void _testCullRectComputation() {
  ...
  test(
    ...
    () async {
      ...
      expect(
        ...
        const ui.Rect.fromLTRB(0, 0, 500, 100),
      );
    },
    skip: '''
  ...
  test('limits to paint bounds if no clip layers', () async {
    ...
    expect(
      picture.optimalLocalCullRect,
      const ui.Rect.fromLTRB(40, 40, 60, 60),
    );
  });
  ...
  test(
    ...
    () async {
      ...
      final PersistedPicture picture = enumeratePictures().single;
      expect(
        picture.debugExactGlobalCullRect,
        const ui.Rect.fromLTRB(0, 70, 20, 100),
      );
      expect(
        picture.optimalLocalCullRect,
        const ui.Rect.fromLTRB(0, -20, 20, 10),
      );
    },
    skip: '''
  TODO(https://github.com/flutter/flutter/issues/40395)
  ...
  test('fills layer clip rect', () async {
    ...
    expect(
      picture.optimalLocalCullRect,
      const ui.Rect.fromLTRB(40, 40, 70, 70),
    );
  });
  ...
  test('intersects layer clip rect and paint bounds', () async {
    ...
    expect(
      picture.optimalLocalCullRect,
      const ui.Rect.fromLTRB(50, 40, 70, 70),
    );
  });
  ...
  test('renders clipped text with high quality', () async {
    ...
    final ui.Rect outerClip = ui.Rect.fromLTRB(
      0.5,
      0.5,
      canvasSize.right,
      canvasSize.bottom,
