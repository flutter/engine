...
Future<void> testMain() async {
  ...
  void testAlignAndTransform(EngineCanvas canvas) {
    ...
    void drawParagraphAt(Offset offset, TextAlign align) {
      ...
      final rect = Rect.fromLTRB(0.0, 0.0, 150.0, paragraph.height);
  ...
  void testGiantParagraphStyles(EngineCanvas canvas) {
    ...
    final rect = Rect.fromLTRB(0.0, 0.0, paragraph.maxIntrinsicWidth, paragraph.height);
    canvas.drawRect(rect, SurfacePaintData()..color = black.value);
    canvas.drawParagraph(paragraph, Offset.zero);
  }
  ...
  test('giant font size on the body tag (DOM)', () async {
    const bounds = Rect.fromLTWH(0, 0, 600, 200);

    // Store the old font size value on the body, and set a gaint font size.
    final oldBodyFontSize = domDocument.body!.style.fontSize;
    domDocument.body!.style.fontSize = '100px';

    final canvas = DomCanvas(domDocument.createElement('flt-picture'));
    var offset = const Offset(10.0, 10.0);
    ...
    final rect = Rect.fromLTWH(offset.dx, offset.dy, paragraph.maxIntrinsicWidth, paragraph.height);
    canvas.drawRect(rect, SurfacePaintData()..color = black.value);
    canvas.drawParagraph(paragraph, offset);
    ...
    final placeholderHeight = paragraph.height;
    final placeholderWidth = paragraph.height * 2;
    ...
    final rect2 = Rect.fromLTWH(offset.dx, offset.dy, paragraph2.maxIntrinsicWidth, paragraph2.height);
  ...
  test('draws text decorations', () {
    final canvas = BitmapCanvas(bounds, RenderStrategy());
    final decorationStyles = <TextDecorationStyle>[
    ...
    final paragraph = rich(
      EngineParagraphStyle(fontFamily: 'Roboto'),
      (CanvasParagraphBuilder builder) {
        for (final decorationStyle in decorationStyles) {
          builder.pushStyle(EngineTextStyle.only(
  ...
  void testFontFeatures(EngineCanvas canvas) {
    const text = 'Bb Difficult ';
    const enableSmallCaps = FontFeature('smcp');
    const disableSmallCaps = FontFeature('smcp', 0);

    const numeric = '123.4560';
    const enableOnum = FontFeature('onum');

    const disableLigatures = FontFeature('liga', 0);
  ...
  test('font features', () {
    const bounds = Rect.fromLTWH(0, 0, 600, 500);
    final canvas = BitmapCanvas(bounds, RenderStrategy());
  ...
  test('paragraph bounds hug the text inside the paragraph', () async {
    const bounds = Rect.fromLTWH(0, 0, 150, 100);

    final builder = CanvasParagraphBuilder(EngineParagraphStyle(
      fontFamily: 'Ahem',
      fontSize: 20,
