...
Iterable<PersistedSurface> enumerateSurfaces([PersistedSurface? root]) {
  ...
  final surfaces = <PersistedSurface>[root!];
...
double _maxComponentColorDistance(Color a, Color b) {
  var delta = math.max<int>((a.red - b.red).abs(), (a.green - b.green).abs());
...
double _sizeDistance(Size a, Size b) {
  final delta = (b - a) as Offset; // ignore: unnecessary_parenthesis
...
class _IsWithinDistance<T> extends Matcher {
  ...
  bool matches(Object? object, Map<dynamic, dynamic> matchState) {
    ...
    final test = object;
    final distance = distanceFunction(test, value);
    if (distance < 0) {
      throw ArgumentError(
...
Matcher hasHtml(String htmlPattern) {
  final originalDom = html.parseFragment(htmlPattern);
...
class HtmlPatternMatcher extends Matcher {
  ...
  bool matches(final Object? object, Map<Object?, Object?> matchState) {
    ...
    final element = html.parseFragment(object.outerHTML).children.single;
  ...
  static bool _areTagsEqual(html.Element a, html.Element b) {
    ...
    var aName = a.localName!.toLowerCase();
    var bName = b.localName!.toLowerCase();
  ...
  void matchElements(_Breadcrumbs parent, List<String> mismatches, html.Element element, html.Element pattern) {
    final breadcrumb = parent.element(pattern.localName!);
  ...
  void matchAttributes(_Breadcrumbs parent, List<String> mismatches, html.Element element, html.Element pattern) {
    for (final attribute in pattern.attributes.entries) {
      final expectedName = attribute.key as String;
      final expectedValue = attribute.value;
      final breadcrumb = parent.attribute(expectedName);
      ...
      if (expectedName == 'style') {
        ...
        if (!element.attributes.containsKey(expectedName)) {
          ...
          final actualValue = element.attributes[expectedName];
  ...
  static Map<String, String> parseStyle(html.Element element) {
    ...
    for (final attribute in rawStyle.split(';')) {
  ...
  void matchStyle(_Breadcrumbs parent, List<String> mismatches, html.Element element, html.Element pattern) {
    final expected = parseStyle(pattern);
    final actual = parseStyle(element);
    for (final entry in expected.entries) {
      final breadcrumb = parent.styleProperty(entry.key);
  ...
  List<html.Node> _cleanUpNodeList(html.NodeList nodeList) {
    ...
    for (var i = 0; i < nodeList.length; i++) {
      ...
      final hasSiblings = nodeList.length > 1;
      final isWhitespace = node is html.Text && node.data.trim().isEmpty;
  ...
  void matchChildren(_Breadcrumbs parent, List<String> mismatches, html.Element element, html.Element pattern) {
    final actualChildNodes = _cleanUpNodeList(element.nodes);
    final expectedChildNodes = _cleanUpNodeList(pattern.nodes);
    ...
    for (var i = 0; i < expectedChildNodes.length; i++) {
      final expectedChild = expectedChildNodes[i];
      final actualChild = actualChildNodes[i];
