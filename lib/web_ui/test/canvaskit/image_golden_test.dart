...
void _testForImageCodecs({required bool useBrowserImageDecoder}) {
  ...
  group('($mode)', () {
    ...
    test('CkAnimatedImage can be explicitly disposed of', () {
      final CkAnimatedImage image = CkAnimatedImage.decodeFromBytes(
        kTransparentImage,
        'test',
      );
    ...
    test('CkImage toByteData', () async {
      ...
      expect(
        (await image.toByteData(format: ui.ImageByteFormat.png)).lengthInBytes,
        greaterThan(0),
      );
    });

    test(
      'toByteData with decodeImageFromPixels on videoFrame formats',
      () async {
    ...
    test('CkImage.clone also clones the VideoFrame', () async {
      final CkBrowserImageDecoder image =
          await CkBrowserImageDecoder.create(
            data: kAnimatedGif,
            debugSource: 'test',
          );
    ...
    test(
      'skiaInstantiateWebImageCodec loads an image from the network',
      () async {
        ...
        final ui.Codec codec =
            await skiaInstantiateWebImageCodec(
              'http://image-server.com/picture.jpg',
              null,
            );
    ...
    test(
      'instantiateImageCodec with multi-frame image does not support targetWidth/targetHeight',
      () async {
        final ui.Codec codec =
            await ui.instantiateImageCodec(
              kAnimatedGif,
    ...
    test(
      'skiaInstantiateWebImageCodec throws exception on request error',
      () async {
        mockHttpFetchResponseFactory = (String url) async {
    ...
    test(
      'skiaInstantiateWebImageCodec throws exception on HTTP error',
      () async {
        try {
    ...
    test('Reports error when failing to decode malformed image data', () async {
      try {
        await ui.instantiateImageCodec(
          Uint8List.fromList(<int>[0xFF, 0xD8, 0xFF, 0xDB, 0x00, 0x00, 0x00]),
        );
        fail('Expected to throw');
      } on ImageCodecException catch (exception) {
        if (!browserSupportsImageDecoder) {
    ...
    test(
      'Provides readable error message when image type is unsupported',
      () async {
        addTearDown(() {
    ...
    test(
      'decodeImageFromPixels upscale when allowUpscaling is false',
      () async {
        Future<ui.Image> testDecodeFromPixels(int width, int height) async {
          final Completer<ui.Image> completer = Completer<ui.Image>();
          ui.decodeImageFromPixels(
    ...
    test('decoded image can be read back from picture', () async {
      ...
      final CkImage snapshot;
      {
        ...
        canvas.drawRect(const ui.Rect.fromLTRB(5, 5, 20, 20), CkPaint());
        canvas.drawImage(image, ui.Offset.zero, CkPaint());
        canvas.drawRect(const ui.Rect.fromLTRB(90, 90, 105, 105), CkPaint());
