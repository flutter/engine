    ...
    final List<DomImageBitmap> bitmaps =
        await Future.wait(
          pictures.map((ScenePicture picture) {
            ...
            final Future<DomImageBitmap> bitmap = createImageBitmap(
...
void testMain() {
  ...
  test('SceneView places canvas according to device-pixel ratio', () async {
    ...
    final StubPicture picture = StubPicture(
    ...
    expect(
      ...
      equalsIgnoringCase('flt-canvas-container'),
    );
  ...
  test(
    'SceneView places platform view according to device-pixel ratio',
    () async {
      debugOverrideDevicePixelRatio(2.0);
      ...
      rootLayer.slices.add(
        PlatformViewSlice(<PlatformView>[platformView], null),
      );
      final EngineScene scene = EngineScene(rootLayer);
      await sceneView.renderScene(scene, null);
      ...
      expect(
        containerElement.tagName,
        equalsIgnoringCase('flt-platform-view-slot'),
      );
  ...
  test('SceneView clips pictures that are outside the window screen', () async {
    final StubPicture picture = StubPicture(
      const ui.Rect.fromLTWH(-50, -50, 100, 120),
    );
