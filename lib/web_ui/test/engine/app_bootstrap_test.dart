...
void testMain() {
  ...
  test('engineInitializer autoStart() does the same as Dart autoStart()', () async {
    ...
    final engineInitializer = bootstrap.prepareEngineInitializer();

    expect(engineInitializer, isNotNull);

    final maybeApp = await promiseToFuture<Object>(callMethod<Object>(engineInitializer, 'autoStart', <Object?>[]));
  ...
  test('engineInitializer initEngine() calls init and returns an appRunner', () async {
    ...
    final maybeAppInitializer = await promiseToFuture<Object>(callMethod<Object>(engineInitializer, 'initializeEngine', <Object?>[]));
  ...
  test('appRunner runApp() calls run and returns a FlutterApp', () async {
    ...
    final appInitializer = await promiseToFuture<Object>(callMethod<Object>(
      engineInitializer,
      'initializeEngine',
      <Object?>[]
    ));
    expect(appInitializer, isA<FlutterAppRunner>());
    final maybeApp = await promiseToFuture<Object>(callMethod<Object>(
      appInitializer,
      'runApp',
      <Object?>[]
    ));
