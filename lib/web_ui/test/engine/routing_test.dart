...
void testMain() {
  ...
  test('EnginePlatformDispatcher.instance.implicitView should be non-null',
      () async {
  ...
  test('window.defaultRouteName should work with a custom url strategy',
      () async {
    ...
    await myWindow.debugInitializeHistory(
        TestUrlStrategy.fromEntry(
          ...
          const TestHistoryEntry(
              'initial state', null, '/not-really-inital/THIS_IS_IGNORED'),
        ),
        useSingle: true);
  ...
  test('can switch history mode', () async {
    ...
    await check<SingleEntryBrowserHistory>(
        'selectSingleEntryHistory', null); // -> single
    await check<MultiEntriesBrowserHistory>(
        'selectMultiEntryHistory', null); // -> multi
    await check<SingleEntryBrowserHistory>(
        'selectSingleEntryHistory', <String, dynamic>{}); // -> single
    await check<MultiEntriesBrowserHistory>(
        'selectMultiEntryHistory', <String, dynamic>{}); // -> multi
    await check<SingleEntryBrowserHistory>(
        'routeUpdated', <String, dynamic>{'routeName': '/bar'}); // -> single
    await check<SingleEntryBrowserHistory>('routeInformationUpdated',
        <String, dynamic>{'location': '/bar'}); // does not change mode
    await check<MultiEntriesBrowserHistory>(
        'selectMultiEntryHistory', <String, dynamic>{}); // -> multi
    await check<MultiEntriesBrowserHistory>('routeInformationUpdated',
        <String, dynamic>{'location': '/bar'}); // does not change mode
  });
  ...
  test('should not throw when state is complex json object', () async {
    ...
    expect(myWindow.browserHistory, isA<MultiEntriesBrowserHistory>());
    expect(myWindow.browserHistory.urlStrategy!.getPath(), '/baz');
    final wrappedState = myWindow.browserHistory.urlStrategy!.getState()!
  ...
  test('routeInformationUpdated can handle uri', () async {
    ...
    await callback.future;
    expect(myWindow.browserHistory, isA<MultiEntriesBrowserHistory>());
    expect(myWindow.browserHistory.urlStrategy!.getPath(),
        '/baz?abc=def#fragment');
  });
  ...
  test('can replace in MultiEntriesBrowserHistory', () async {
    ...
    await callback.future;
    expect(myWindow.browserHistory.urlStrategy!.getPath(), '/baz');
    expect(myWindow.browserHistory.urlStrategy!.getState(),
        _tagStateWithSerialCount('/state', 1));
    ...
    expect(myWindow.browserHistory.urlStrategy!.getPath(), '/baz');
    expect(myWindow.browserHistory.urlStrategy!.getState(),
        _tagStateWithSerialCount('/state1', 1));
  ...
  test('initialize browser history with default url strategy (single)',
      () async {
  ...
  test('initialize browser history with default url strategy (multiple)',
      () async {
    // On purpose, we don't initialize history on the window. We want to let the
    // window to self-initialize when it receives a navigation message.
