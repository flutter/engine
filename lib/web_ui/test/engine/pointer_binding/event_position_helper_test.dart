...
void doTests() {
  ...
  group('computeEventOffsetToTarget', () {
    ...
    test('Event dispatched by target returns offsetX, offsetY', () async {
      ...
      final DomMouseEvent event =
          await dispatchAndCatch(
            rootElement,
    ...
    test(
      'Event dispatched on child re-computes offset (offsetX/Y invalid)',
      () async {
        // Fire an event contained within target...
        final DomMouseEvent event =
            await dispatchAndCatch(
    ...
    test(
      'Event dispatched by TalkBack gets a computed offset',
      () async {
