...
void testMain() {
  ...
  group('SceneBuilder', () {
    test('pushOffset implements surface lifecycle', () {
      testLayerLifeCycle(
        ...
        return sceneBuilder.pushOffset(10, 20,
            oldLayer: oldLayer as ui.OffsetEngineLayer?);
      }, () {
    ...
    test('pushTransform implements surface lifecycle', () {
      testLayerLifeCycle(
          (ui.SceneBuilder sceneBuilder, ui.EngineLayer? oldLayer) {
        return sceneBuilder.pushTransform((Matrix4.identity()
    ...
    test('pushClipPath implements surface lifecycle', () {
      testLayerLifeCycle(
        ...
        final ui.Path path = ui.Path()
          ..addRect(const ui.Rect.fromLTRB(10, 20, 30, 40));
        return sceneBuilder.pushClipPath(path,
            oldLayer: oldLayer as ui.ClipPathEngineLayer?);
      }, () {
        return '''
<flt-scene>
  <flt-clippath>
    ...
    test('pushOpacity implements surface lifecycle', () {
      testLayerLifeCycle(
        ...
        return sceneBuilder.pushOpacity(10,
            oldLayer: oldLayer as ui.OpacityEngineLayer?);
      }, () {
...
<flt-scene>
  ...
  group('parent child lifecycle', () {
    test(
      ...
      final PersistedClipRect clip1 = PersistedClipRect(
          null, const ui.Rect.fromLTRB(10, 10, 20, 20), ui.Clip.antiAlias);
      final PersistedOpacity opacity =
          PersistedOpacity(null, 100, ui.Offset.zero);
      ...
      final PersistedClipRect clip2 = PersistedClipRect(
          clip1, const ui.Rect.fromLTRB(10, 10, 20, 20), ui.Clip.antiAlias);
      ...
      final PersistedClipRect clip3 = PersistedClipRect(
          clip2, const ui.Rect.fromLTRB(10, 10, 50, 50), ui.Clip.antiAlias);
  ...
  group('Compositing order', () {
    ...
    test('Canvas element should retain -1 zIndex after update', () async {
      ...
      builder2.pushClipRect(const ui.Rect.fromLTRB(5, 10, 300, 300),
          oldLayer: oldLayer);
    ...
    test('Multiple canvas elements should retain zIndex after update',
        () async {
      final SurfaceSceneBuilder builder = SurfaceSceneBuilder();
      ...
      final DomElement contentAfterReuse = builder2.build().webOnlyRootElement!;
      final List<DomCanvasElement> list = contentAfterReuse
  ...
  test('Should retain same image element', () async {
    ...
    final DomElement contentAfterReuse = builder2.build().webOnlyRootElement!;
    list = contentAfterReuse
        .querySelectorAll('img')
        .cast<DomHTMLImageElement>()
        .toList();
    for (final DomHTMLImageElement image in list) {
  ...
  test('skips painting picture when picture fully clipped out', () async {
    ...
    // Picture not clipped out, so we should see a `<flt-canvas>`
    {
      ...
      final DomElement content = builder.build().webOnlyRootElement!;
      expect(
          content.querySelectorAll('flt-picture').single.children, isNotEmpty);
    }
  ...
  test(
      'releases old canvas when picture is fully clipped out after addRetained',
      () async {
    final ui.Picture picture = _drawPicture();

    // Frame 1: picture visible
    final SurfaceSceneBuilder builder1 = SurfaceSceneBuilder();
    final PersistedOffset offset1 =
        builder1.pushOffset(0, 0) as PersistedOffset;
    builder1.addPicture(ui.Offset.zero, picture);
    builder1.pop();
    final DomElement content1 = builder1.build().webOnlyRootElement!;
    expect(
        content1.querySelectorAll('flt-picture').single.children, isNotEmpty);
    expect(findPictureSurfaceChild(offset1)!.canvas, isNotNull);

    // Frame 2: picture is clipped out after an update
    final SurfaceSceneBuilder builder2 = SurfaceSceneBuilder();
    final PersistedOffset offset2 = builder2.pushOffset(-10000, -10000,
        oldLayer: offset1) as PersistedOffset;
  ...
  test(
    ...
    final PersistedOffset offset1 =
        builder1.pushOffset(0, 0) as PersistedOffset;
    final PersistedOffset subOffset1 =
        builder1.pushOffset(0, 0) as PersistedOffset;
    builder1.addPicture(ui.Offset.zero, picture);
    builder1.pop();
    builder1.pop();
  ...
  test('updates child lists efficiently', () async {
    ...
    ui.EngineLayer pushChild(SurfaceSceneBuilder builder, String char,
      ...
      final ui.EngineLayer newLayer = useOffset
          ? builder.pushOffset(0, 0,
              oldLayer:
                  oldLayer == null ? null : oldLayer as ui.OffsetEngineLayer)
          : builder.pushOpacity(100,
              oldLayer:
                  oldLayer == null ? null : oldLayer as ui.OpacityEngineLayer);
    ...
    final Map<String, ui.EngineLayer> renderedLayers =
        <String, ui.EngineLayer>{};
    ...
    Future<void> testCase(String string, String description,
        {int deletions = 0, int additions = 0, int moves = 0}) {
      ...
      final DomMutationObserver observer =
          createDomMutationObserver((JSArray<JSAny?> mutations, _) {
        for (final DomMutationRecord record
            in mutations.toDart.cast<DomMutationRecord>()) {
          actualDeletions.addAll(record.removedNodes!);
          actualAdditions.addAll(record.addedNodes!);
        }
      });
      observer.observe(SurfaceSceneBuilder.debugLastFrameScene!.rootElement!,
          childList: true);
      ...
      for (int i = 0; i < string.length; i++) {
        ...
        renderedLayers[char] =
            pushChild(builder, char, oldLayer: renderedLayers[char]);
      }
      ...
      renderedLayers.removeWhere(
          (String key, ui.EngineLayer value) => !string.contains(key));
      ...
      return Future<void>.delayed(Duration.zero).then((_) {
        ...
        final int actualMoves =
            actualAdditions.intersection(actualDeletions).length;
  ...
  test('Canvas should allocate fewer pixels when zoomed out', () async {
    ...
    final DomCanvasElement canvas =
        content.querySelector('canvas')! as DomCanvasElement;
    ...
    final DomCanvasElement canvas2 =
        contentAfterScale.querySelector('canvas')! as DomCanvasElement;
  ...
  test('Should recycle canvas once', () async {
    ...
    builder3.pushClipRect(const ui.Rect.fromLTRB(25, 10, 300, 300),
        oldLayer: oldLayer2);
...
class MockPersistedPicture extends PersistedPicture {
  ...
  final BitmapCanvas _fakeCanvas =
      BitmapCanvas(const ui.Rect.fromLTRB(0, 0, 10, 10), RenderStrategy());
