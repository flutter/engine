    ...
    test('pushOffset implements surface lifecycle', () {
      testLayerLifeCycle(
        (ui.SceneBuilder sceneBuilder, ui.EngineLayer? oldLayer) {
          return sceneBuilder.pushOffset(
            10,
            20,
...
''';
    ...
    test('pushClipPath implements surface lifecycle', () {
      testLayerLifeCycle(
        (ui.SceneBuilder sceneBuilder, ui.EngineLayer? oldLayer) {
          final ui.Path path =
              ui.Path()..addRect(const ui.Rect.fromLTRB(10, 20, 30, 40));
          return sceneBuilder.pushClipPath(
            path,
...
''';
    ...
    test('pushOpacity implements surface lifecycle', () {
      testLayerLifeCycle(
        (ui.SceneBuilder sceneBuilder, ui.EngineLayer? oldLayer) {
          return sceneBuilder.pushOpacity(
            10,
...
<flt-scene>
  ...
  group('parent child lifecycle', () {
    test(
      ...
      () {
        ...
        final PersistedOpacity opacity = PersistedOpacity(
          null,
          100,
          ui.Offset.zero,
        );
  ...
  group('Compositing order', () {
    ...
    test(
      'Multiple canvas elements should retain zIndex after update',
      () async {
        final SurfaceSceneBuilder builder = SurfaceSceneBuilder();
        ...
        final DomElement contentAfterReuse =
            builder2.build().webOnlyRootElement!;
  ...
  test('skips painting picture when picture fully clipped out', () async {
    ...
    // Picture not clipped out, so we should see a `<flt-canvas>`
    {
      ...
      final DomElement content = builder.build().webOnlyRootElement!;
      expect(
        content.querySelectorAll('flt-picture').single.children,
        isNotEmpty,
      );
    }
  ...
  test(
    ...
    () async {
      ...
      expect(
        content1.querySelectorAll('flt-picture').single.children,
        isNotEmpty,
      );
  ...
  test('updates child lists efficiently', () async {
    ...
    ui.EngineLayer pushChild(
      ...
      final ui.Paragraph paragraph =
          (ui.ParagraphBuilder(ui.ParagraphStyle())
                ..pushStyle(
                  ui.TextStyle(decoration: ui.TextDecoration.lineThrough),
                )
                ..addText(char))
              .build();
    ...
    Future<void> testCase(
      ...
      observer.observe(
        SurfaceSceneBuilder.debugLastFrameScene!.rootElement!,
        childList: true,
      );
      ...
      for (int i = 0; i < string.length; i++) {
        ...
        renderedLayers[char] = pushChild(
          builder,
          char,
          oldLayer: renderedLayers[char],
        );
      }
...
typedef TestLayerBuilder =
    ui.EngineLayer Function(
      ui.SceneBuilder sceneBuilder,
      ui.EngineLayer? oldLayer,
    );
typedef ExpectedHtmlGetter = String Function();

void testLayerLifeCycle(
  TestLayerBuilder layerBuilder,
  ExpectedHtmlGetter expectedHtmlGetter,
) {
...
HtmlImage createTestImage({int width = 100, int height = 50}) {
  ...
  imageElement.src = js_util.callMethod<String>(
    canvas,
    'toDataURL',
    <dynamic>[],
  );
