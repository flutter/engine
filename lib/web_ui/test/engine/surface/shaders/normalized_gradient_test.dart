...
void testMain() {
  group('Shader Normalized Gradient', () {
    test('3 stop at start', () {
      final gradient = NormalizedGradient(const <ui.Color>[
        ui.Color(0xFF000000), ui.Color(0xFFFF7f3f)
      ], stops: <double>[0.0, 0.5]);
      var res = _computeColorAt(gradient, 0.0);
...
int _computeColorAt(NormalizedGradient gradient, double t) {
  ...
  final r = t * gradient.scaleAt(i * 4) + gradient.biasAt(i * 4);
  final g = t * gradient.scaleAt(i * 4 + 1) + gradient.biasAt(i * 4 + 1);
  final b = t * gradient.scaleAt(i * 4 + 2) + gradient.biasAt(i * 4 + 2);
  final a = t * gradient.scaleAt(i * 4 + 3) + gradient.biasAt(i * 4 + 3);
  var val = 0;
