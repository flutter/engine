...
void testMain() {
  group('ImageFilter constructors', () {
    ...
    test('matrix tests all values on ==', () {
      final Matrix4 matrix = Matrix4.identity();
      final Float64List storage = matrix.toFloat64();
      final ImageFilter filter1a = ImageFilter.matrix(
        storage,
        filterQuality: FilterQuality.none,
      );
      final ImageFilter filter1b = ImageFilter.matrix(
        storage,
        filterQuality: FilterQuality.high,
      );

      storage[0] = 2.0;
      final ImageFilter filter2a = ImageFilter.matrix(
        storage,
        filterQuality: FilterQuality.none,
      );
      final ImageFilter filter2b = ImageFilter.matrix(
        storage,
        filterQuality: FilterQuality.high,
      );
