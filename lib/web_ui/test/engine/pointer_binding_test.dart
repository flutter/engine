...
void testMain() {
  ...
  test('allows default on touchstart events', () async {
    ...
    expect(
      ...
      isFalse,
      reason:
          'touchstart events should NOT be prevented. That breaks semantic taps!',
    );
  });
  ...
  test(
    ...
    () {
      ...
      keyboardConverter
          .handleEvent(keyDownEvent('ControlLeft', 'Control', kCtrl));
      ...
      keyboardConverter
          .handleEvent(keyUpEvent('ControlLeft', 'Control', kCtrl));
      ...
      expect(packets[1].data[0].physicalDeltaX, equals(0.0));
      expect(packets[1].data[0].physicalDeltaY, equals(0.0));
      expect(packets[1].data[0].scale,
          closeTo(0.60653065971, 1e-10)); // math.exp(-100/200)
  ...
  group('Listener', () {
    ...
    test('listeners are registered only once', () {
      ...
      eventTarget.dispatchEvent(expected);
      expect(timesHandled, 1,
          reason: 'The handler ran multiple times for a single event.');
    });
  });
...
void _testClickDebouncer({required PointerBinding Function() getBinding}) {
  ...
  testWithSemantics('Accumulates pointer events starting from pointerdown',
      () async {
    expect(EnginePlatformDispatcher.instance.semanticsEnabled, true);
    expect(PointerBinding.clickDebouncer.isDebouncing, false);
    ...
    expect(
      PointerBinding.clickDebouncer.debugState!.queue
          .map<String>((QueuedEvent e) => e.event.type),
  ...
  testWithSemantics('Flushes events to framework when target changes',
      () async {
    expect(EnginePlatformDispatcher.instance.semanticsEnabled, true);
    expect(PointerBinding.clickDebouncer.isDebouncing, false);
  ...
  testWithSemantics(
    ...
    expect(semanticsActions,
        <CapturedSemanticsEvent>[(type: ui.SemanticsAction.tap, nodeId: 42)]);
  });

  testWithSemantics('Forwards click to framework when debouncing and listening',
      () async {
    expect(PointerBinding.clickDebouncer.isDebouncing, false);
  ...
  testWithSemantics('Dedupes click if pointer down/up flushed recently',
      () async {
    expect(EnginePlatformDispatcher.instance.semanticsEnabled, true);
    expect(PointerBinding.clickDebouncer.isDebouncing, false);
  ...
  testWithSemantics(
    ...
    expect(
      reason:
          'The "pointerup" should be forwarded to the framework immediately',
      pointerPackets,
  ...
  DomEvent wheel({
    ...
    if (timeStamp != null) {
      js_util.callMethod<void>(objectConstructor, 'defineProperty', <dynamic>[
        ...
        'timeStamp',
        js_util
            .jsify(<String, dynamic>{'value': timeStamp, 'configurable': true})
      ]);
    }
    return event;
  }
