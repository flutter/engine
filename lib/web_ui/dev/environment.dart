...
class Environment {
  factory Environment() {
    ...
    final io.Directory outDir = io.Directory(
      pathlib.join(engineSrcDir.path, 'out'),
    );
    final io.Directory wasmReleaseOutDir = io.Directory(
      pathlib.join(outDir.path, 'wasm_release'),
    );
    final io.Directory wasmProfileOutDir = io.Directory(
      pathlib.join(outDir.path, 'wasm_profile'),
    );
  ...
  String get dartAotRuntimePath => pathlib.join(
    dartSdkDir.path,
    'bin',
    'dartaotruntime',
  );
  ...
  String get dart2wasmSnapshotPath => pathlib.join(
    dartSdkDir.path,
    'bin',
    'snapshots',
  ...
  io.Directory get flutterDirectory => io.Directory(
    pathlib.join(engineSrcDir.path, 'flutter'),
  );
  io.Directory get webSdkRootDir => io.Directory(
    pathlib.join(flutterDirectory.path, 'web_sdk'),
  );

  /// Path to the "web_engine_tester" package.
  io.Directory get webEngineTesterRootDir => io.Directory(
    pathlib.join(webSdkRootDir.path, 'web_engine_tester'),
  );

  /// Path to the "build" directory, generated by "package:build_runner".
  ///
  /// This is where compiled test output goes.
  io.Directory get webUiBuildDir => io.Directory(
    pathlib.join(outDir.path, 'web_tests'),
  );

  io.Directory get webTestsArtifactsDir => io.Directory(
    pathlib.join(webUiBuildDir.path, 'artifacts'),
  );

  /// Path to the ".dart_tool" directory, generated by various Dart tools.
  io.Directory get webUiDartToolDir => io.Directory(
    pathlib.join(webUiRootDir.path, '.dart_tool'),
  );
  ...
  io.Directory get engineDartToolDir => io.Directory(
    pathlib.join(engineSrcDir.path, 'flutter', '.dart_tool'),
  );

  /// Path to the "dev" directory containing engine developer tools and
  /// configuration files.
  io.Directory get webUiDevDir => io.Directory(
    pathlib.join(webUiRootDir.path, 'dev'),
  );

  /// Path to the "test" directory containing web engine tests.
  io.Directory get webUiTestDir => io.Directory(
    pathlib.join(webUiRootDir.path, 'test'),
  );

  /// Path to the "lib" directory containing web engine code.
  io.Directory get webUiLibDir => io.Directory(
    pathlib.join(webUiRootDir.path, 'lib'),
  );

  /// Path to the base directory to be used by Skia Gold.
  io.Directory get webUiSkiaGoldDirectory => io.Directory(
    pathlib.join(webUiDartToolDir.path, 'skia_gold'),
  );

  /// Directory to add test results which would later be uploaded to a gcs
  /// bucket by LUCI.
  io.Directory get webUiTestResultsDirectory => io.Directory(
