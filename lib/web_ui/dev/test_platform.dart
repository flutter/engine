...
class BrowserPlatform extends PlatformPlugin {
  ...
  Future<shelf.Response> _screeshotHandler(shelf.Request request) async {
    ...
    final double maxRateDiff = requestData['maxratediff'];
    final Map<String, dynamic> region = requestData['region'];
    final String result = await _diffScreenshot(filename, write, maxRateDiff ?? _kMaxDiffRateFailure, region);
    return shelf.Response.ok(json.encode(result));
  }

  Future<String> _diffScreenshot(String filename, bool write, double maxRateDiffFailure, [ Map<String, dynamic> region ]) async {
...
''');
      ...
      message.writeln(getPrintableDiffFilesInfo(diff.rate, maxRateDiffFailure));
      ...
      if (diff.rate < maxRateDiffFailure) {
