...
class BrowserPlatform extends PlatformPlugin {
  BrowserPlatform._(
    ...
    final shelf.Cascade cascade = shelf.Cascade()
        ...
        .add(
          createSimpleDirectoryHandler(
            getBundleBuildDirectory(suite.testBundle),
          ),
        )
        ...
        .add(
          createSimpleDirectoryHandler(
            getTestSetDirectory(suite.testBundle.testSet),
          ),
        )
  ...
  Future<shelf.Response> _canvasKitOverrideHandler(
    ...
    final File file = File(
      p.joinAll(<String>[pathOverride, ...p.split(request.url.path).skip(1)]),
    );
    ...
    return shelf.Response.ok(file.readAsBytesSync(), headers: <String, Object>{
  ...
  Future<shelf.Response> _testImageListingHandler(shelf.Request request) async {
    ...
    final List<String> testImageFiles =
        testImageDirectory
            ...
            .whereType<File>()
            .map<String>(
              (File file) => p.relative(
                file.path,
                from: testImageDirectory.path,
              ),
            )
            .where(
              (String path) => supportedImageTypes.containsKey(
                p.extension(path),
              ),
            )
            .toList();

    return shelf.Response.ok(json.encode(testImageFiles), headers:
        <String, Object>{HttpHeaders.contentTypeHeader: 'application/json'});
  }
  ...
  shelf.Handler _createAbsolutePackageUrlHandler() {
    ...
    for (final Package package in packageConfig.packages) {
      ...
      final String rootRelativePath = p.relative(
        configPath,
        from: p.rootPrefix(configPath),
      );
  ...
  Future<shelf.Response> _testPayloadGenerator(shelf.Request request) async {
    ...
    final int payloadLength = int.parse(
      request.requestedUri.queryParameters['length']!,
    );
    final int chunkLength = int.parse(
      request.requestedUri.queryParameters['chunk']!,
    );
    ...
    return shelf.Response.ok(controller.stream, headers: <String, String>{
  ...
  Future<shelf.Response> _screenshotHandler(shelf.Request request) async {
    ...
    final String result =
        await _diffScreenshot(filename, region, isCanvaskitTest);
  ...
  shelf.Handler createSimpleDirectoryHandler(Directory directory) {
    return (shelf.Request request) {
      final File fileInDirectory = File(
        p.join(directory.path, request.url.path),
      );
      ...
      final bool isScript =
          extension == '.js' || extension == '.mjs' || extension == '.html';
      return shelf.Response.ok(fileInDirectory.readAsBytesSync(), headers:
          <String, Object>{
            HttpHeaders.contentTypeHeader: contentType,
            if (isScript && needsCrossOriginIsolated) ...coopCoepHeaders,
          });
    };
  }
  ...
  shelf.Response _testBootstrapHandler(shelf.Request request) {
    ...
    if (path.endsWith('.html')) {
      ...
      final String buildConfigsString = suite.testBundle.compileConfigs
          .map(
            (CompileConfiguration config) => _makeBuildConfigString(
              scriptBase,
              config,
            ),
          )
...
''';
      ...
      ''',
        headers: <String, String>{
          ...
          if (needsCrossOriginIsolated) ...coopCoepHeaders,
        },
      );
    }
  ...
  Future<RunnerSuite> load(
    ...
    final Uri suiteUrl = url.resolveUri(
      p.toUri('${p.withoutExtension(path)}.html'),
    );
    _checkNotClosed();
  ...
  Future<BrowserManager?> _startBrowserManager() {
    ...
    final Uri hostUrl = url.resolve('host/index.html').replace(queryParameters:
        <String, dynamic>{
    ...
    final Future<BrowserManager?> future = BrowserManager.start(
      ...
      debug: isDebug,
      sourceMapDirectory:
          hasSourceMaps ? getBundleBuildDirectory(suite.testBundle) : null,
    );
...
class BrowserManager {
  ...
  BrowserManager._(
    ...
    _channel = MultiChannel<dynamic>(
      webSocket.cast<String>().transform(jsonDocument).changeStream((
  ...
  Future<RunnerSuite> load(
    ...
    final StreamChannel<dynamic> suiteChannel = virtualChannel.transformStream(
      StreamTransformer<dynamic, dynamic>.fromHandlers(handleDone: (
  ...
  Future<Image> captureScreenshot(Rectangle<num> region) => _browser
