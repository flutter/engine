...
namespace {
std::vector<std::shared_ptr<fml::Mapping>> ShaderLibraryMappingsForApplication() {
    return {
...
public:
    ReactorWorker() {
        emscripten_console_log("hi worker\n");
    }

    ~ReactorWorker() override {
        emscripten_console_log("bye worker\n");
    }
    ...
    virtual bool CanReactorReactOnCurrentThreadNow(const impeller::ReactorGLES& reactor) const override {
        return true;
    }
};
...
sk_sp<GraphicsContext> createGraphicsContext() {
    ...
    emscripten_console_log(static_cast<std::shared_ptr<impeller::Context>>(context)->IsValid() ? "context valid" : "context invalid");
    ...
    return sk_make_sp<GraphicsContext>(std::move(context), std::move(renderer), std::move(worker));
};

GraphicsContext::GraphicsContext(std::shared_ptr<impeller::ContextGLES> context, std::shared_ptr<impeller::Renderer> renderer, std::shared_ptr<ReactorWorker> worker) : 
...
void GraphicsSurface::renderPicture(const impeller::Picture& picture) {
    ...
    auto surface = impeller::SurfaceGLES::WrapFBO(_context,                        // context
    ...
    bool result = _renderer->Render(std::move(surface), [this, &picture](impeller::RenderTarget& target) {
